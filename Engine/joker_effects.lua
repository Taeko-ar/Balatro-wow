...
local function rank_is_face(rank)
    rank = tonumber(rank)
    return rank == 11 or rank == 12 or rank == 13 or G:hasJoker("j_pareidolia")
end
...
local function held_cards(ctx)
    local out = {}
    local played = {}
    for _, n in ipairs((ctx and ctx.played_cards) or {}) do
        played[n] = true
    end
    ...
    for _, node in ipairs(hand_nodes) do
        if not played[node] and node and node.card_data then
            table.insert(out, node.card_data)
        end
    end
    return out
end
...
local function blueprint_immediate_target(joker)
    if type(G and G.jokers) ~= "table" then
        return nil
    end
...
local function resolve_copy_target(src, visited)
    ...
    if id == "j_brainstorm" then
        ...
        if type(next_src) ~= "table" or next_src == src then
            return nil
        end
    ...
    if not is_blueprint_copy_target(src) then
        return nil
    end
    return src
end
...
local function delegate_joker_effect(delegator, src, ctx)
    ...
    if type(resolved) ~= "table" or resolved == delegator then
        return
    end
    if type(resolved.apply_effect) ~= "function" then
        return
    end
    ...
    if type(ctx) == "table" then
        ctx._suppress_joker_apply_shake = true
    end
...
local function delegate_joker_retrigger(delegator, src, ctx)
    ...
    if type(resolved.query_retrigger) ~= "function" then
        return 0
    end
...
local function first_scoring_play_node(played_cards)
    if type(played_cards) ~= "table" then
        return nil
    end
...
local SPECIAL = {
    j_joker = {
        matches_trigger = function(_, e)
            return e == "on_hand_scored"
        end,
    ...
    j_greedy_joker = {
        matches_trigger = function(_, e)
            return e == "card_played"
        end,
        apply_effect = function(_, ctx)
            if is_suit(ctx.suit, "Diamonds") then
                add_mult(ctx, 3)
            end
        end,
    },
    j_lusty_joker = {
        ...
        apply_effect = function(_, ctx)
            if is_suit(ctx.suit, "Hearts") then
                add_mult(ctx, 3)
            end
        end,
    },
    j_wrathful_joker = {
        ...
        apply_effect = function(_, ctx)
            if is_suit(ctx.suit, "Spades") then
                add_mult(ctx, 3)
            end
        end,
    },
    j_gluttenous_joker = {
        ...
        apply_effect = function(_, ctx)
            if is_suit(ctx.suit, "Clubs") then
                add_mult(ctx, 3)
            end
        end,
    },
    j_jolly = {
        ...
        apply_effect = function(_, ctx)
            if has_hand_type(ctx, "Pair") then
                add_mult(ctx, 8)
            end
        end,
    },
    j_zany = {
        ...
        apply_effect = function(_, ctx)
            if has_hand_type(ctx, "Three of a Kind") then
                add_mult(ctx, 12)
            end
        end,
    },
    j_mad = {
        ...
        apply_effect = function(_, ctx)
            if has_hand_type(ctx, "Two Pair") then
                add_mult(ctx, 10)
            end
        end,
    },
    ...
    j_droll = {
        ...
        apply_effect = function(_, ctx)
            if has_hand_type(ctx, "Flush") then
                add_mult(ctx, 10)
            end
        end,
    },
    ...
    j_stencil = {
        matches_trigger = function(j, e, ctx)
            ...
            if ctx == nil or tonumber(ctx.free_joker_slots) == nil then
                return false
            end
    ...
    j_hanging_chad = {
        query_retrigger = function(joker, ctx)
            ...
            if not first or node ~= first then
                return 0
            end
    ...
    j_blueprint = {
        matches_trigger = function(_, _, _)
            return true
        end,
    ...
    j_abstract = {
        ...
        apply_effect = function(_, ctx)
            add_mult(ctx, 3 * ((G and G.jokers and #G.jokers) or 0))
        end,
    },
    j_supernova = {
        matches_trigger = function(_, e)
    ...
    j_banner = {
        ...
        apply_effect = function(_, ctx)
            add_chips(ctx, 30 * (tonumber(ctx.discards_left) or tonumber(G and G.discards) or 0))
        end,
    },
    j_blue_joker = {
        matches_trigger = function(_, e)
        ...
        apply_effect = function(_, ctx)
            add_chips(ctx, 2 * count_cards_in_deck())
        end,
    },
    j_stone_joker = {
        matches_trigger = function(_, e)
        ...
        apply_effect = function(_, ctx)
            if count_full_deck(function(c)
                return c.enhancement == "stone"
            end) > 0 then
                add_chips(ctx, 25 * count_full_deck(function(c)
                    return c.enhancement == "stone"
                end))
            end
        end,
    ...
    j_bull = {
        ...
        apply_effect = function(_, ctx)
            add_chips(ctx, 2 * (tonumber(G and G.money) or 0))
        end,
    },
    j_bootstraps = {
        matches_trigger = function(_, e)
        ...
        apply_effect = function(_, ctx)
            add_mult(ctx, math.floor((tonumber(G and G.money) or 0) / 5) * 2)
        end,
    },
    j_green_joker = {
        matches_trigger = function(_, e)
            return e == "on_hand_played" or e == "on_discard" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_hand_played" then
                j.stored_mult = (tonumber(j.stored_mult) or 0) + 1
            elseif ctx.event_name == "on_discard" and ctx.discard_reason == "discard" then
    ...
    j_runner = {
        ...
        apply_effect = function(j, ctx)
            if has_hand_type(ctx, "Straight") then
                j.stored_chips = (tonumber(j.stored_chips) or 0) + 15
            end
    ...
    j_square = {
        ...
        apply_effect = function(j, ctx)
            if type(ctx.cards) == "table" and #ctx.cards == 4 then
    ...
    j_spare_trousers = {
        ...
        apply_effect = function(j, ctx)
            if has_hand_type(ctx, "Two Pair") then
                j.stored_mult = (tonumber(j.stored_mult) or 0) + 2
            end
            add_mult(ctx, tonumber(j.stored_mult) or 0)
        end,
    },
    j_flash_card = {
        matches_trigger = function(_, e)
            return e == "on_shop_reroll" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_shop_reroll" then
                j.stored_mult = (tonumber(j.stored_mult) or 0) + 2
            else
                add_mult(ctx, tonumber(j.stored_mult) or 0)
            end
    ...
    j_popcorn = {
        matches_trigger = function(_, e)
            return e == "on_round_end" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_constellation = {
        matches_trigger = function(_, e)
            return e == "on_consumable_used" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_hologram = {
        matches_trigger = function(_, e)
            return e == "on_cards_added_to_deck" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_lucky_cat = {
        matches_trigger = function(_, e)
            return e == "lucky_trigger" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_campfire = {
        matches_trigger = function(_, e)
            return e == "on_joker_sold" or e == "on_round_end" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_egg = {
        matches_trigger = function(_, e)
            return e == "on_round_end"
        end,
        apply_effect = function(j, ctx)
            j.sell_cost = (tonumber(j.sell_cost) or 0) + 3
            mark_effect_applied(ctx)
        end,
    },
    j_golden_joker = {
    ...
    j_cloud_9 = {
        ...
        apply_effect = function(j, ctx)
            add_round_win_money(
                ctx,
                j,
                count_full_deck(function(c)
                    return tonumber(c.rank) == 9
                end)
            )
        end,
    },
    j_to_the_moon = {
        ...
        apply_effect = function(j, ctx)
            add_round_win_money(ctx, j, math.floor((tonumber(G and G.money) or 0) / 5))
        end,
    },
    j_reserved_parking = {
        matches_trigger = function(_, e)
        ...
        apply_effect = function(_, ctx)
            if rank_is_face(ctx.rank) and G:do_random(1, 2, 1) then
                add_money(ctx, 1)
            end
        end,
    },
    ...
    j_bloodstone = {
        ...
        apply_effect = function(_, ctx)
            if is_suit(ctx.suit, "Hearts") and G:do_random(1, 2, 1) then
                mul_mult(ctx, 1.5)
            end
        end,
    },
    j_8_ball = {
        ...
        apply_effect = function(joker, ctx)
            if tonumber(ctx.rank) ~= 8 then
                return
            end
            local odds = tonumber((joker.effect_config or {}).extra) or 4
            odds = math.max(2, odds)
            if not G:do_random(1, odds, 1) then
                return
            end
            if not G or not G.can_add_consumable or not G.add_consumable or not G.random_consumable_id_of_kind then
                return
            end
            if not G:can_add_consumable() then
                return
            end
    ...
    j_riff_raff = {
        ...
        apply_effect = function(_, ctx)
            if not (G and G.add_joker_by_def and G.random_joker_def_id_by_rarity) then
                return
            end
            for _ = 1, 2 do
    ...
    j_family = {
        ...
        apply_effect = function(_, ctx)
            if has_hand_type(ctx, "Four of a Kind") then
                mul_mult(ctx, 4)
            end
        end,
    },
    ...
    j_wee = {
        matches_trigger = function(_, e)
            return e == "card_played" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_flower_pot = {
        ...
        apply_effect = function(_, ctx)
            ...
            for _, n in ipairs((ctx and ctx.cards) or {}) do
                ...
                if s then
                    if is_suit(s, "Hearts") then
                        has.Hearts = true
                    end
                    if is_suit(s, "Clubs") then
                        has.Clubs = true
                    end
                    if is_suit(s, "Diamonds") then
                        has.Diamonds = true
                    end
                    if is_suit(s, "Spades") then
                        has.Spades = true
                    end
                end
            end
    ...
    j_ticket = {
        ...
        apply_effect = function(_, ctx)
            local cd = ctx.card_node and ctx.card_node.card_data
            if cd and cd.enhancement == "gold" then
                add_money(ctx, 4)
            end
        end,
    },
    j_photograph = {
        matches_trigger = function(_, e)
            return e == "card_played" or e == "on_hand_played"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_hand_played" then
                return
    ...
    j_steel_joker = {
        ...
        apply_effect = function(_, ctx)
            mul_mult(ctx, 1 + 0.2 * count_full_deck(function(c)
                return c.enhancement == "steel"
            end))
        end,
    },
    j_blackboard = {
        ...
        apply_effect = function(_, ctx)
            ...
            for _, c in ipairs(cards) do
                if not is_suit(c.suit, "Spades") and not is_suit(c.suit, "Clubs") then
                    return
                end
            end
            mul_mult(ctx, 3)
    ...
    j_drivers_license = {
        ...
        apply_effect = function(_, ctx)
            local enhanced = count_full_deck(function(c)
    ...
    j_yorick = {
        matches_trigger = function(_, e)
            return e == "on_discard" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_canio = {
        matches_trigger = function(_, e)
            return e == "on_destroy" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_trading_card = {
        matches_trigger = function(_, e, ctx)
            return e == "on_discard" and ctx.discard_reason == "discard"
        end,
        apply_effect = function(_, ctx)
            local discardCount = G:get_effective_discards_per_round() - 1
            local discarded = ctx.discarded_cards
            if type(discarded) ~= "table" or #discarded ~= 1 or discardCount ~= G.discards then
                return
            end
            local deck = G and G.deck
            local pile = deck and deck.discard_pile
            if type(pile) ~= "table" or #pile < 1 then
                return
            end
    ...
    j_ancient_joker = {
        matches_trigger = function(_, e)
            return e == "card_played" or e == "on_round_end"
        end,
        apply_effect = function(j, ctx)
    ...
    j_gros_michel = {
        matches_trigger = function(_, e)
            return e == "on_hand_scored" or e == "on_round_end"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_hand_scored" then
                add_mult(ctx, 15)
    ...
    j_space = {
        matches_trigger = function(_, e)
            return e == "on_hand_played"
        end,
        apply_effect = function(_, ctx)
            if not G:do_random(1, 4, 1) then
                return
            end
            local idx = ctx and tonumber(ctx.hand_index)
            if not G or not idx or not G.upgrade_hand_level_at_index then
                return
            end
    ...
    j_dna = {
        ...
        apply_effect = function(_, ctx)
            ...
            if type(ctx.cards) ~= "table" or #ctx.cards ~= 1 then
                return
            end
            local eff = G and G.get_effective_hands_per_round and G:get_effective_hands_per_round()
            print(eff .. " : " .. G.hands)
            if (tonumber(G and G.hands) or 0) ~= eff - 1 then
                return
            end
            ...
            local hand = G and G.hand
            if not cd or not hand or not hand.add_card then
                return
            end
            ...
            if G.ensure_card_uid then
                G:ensure_card_uid(copy)
            end
    ...
    j_sixth_sense = {
        ...
        apply_effect = function(_, ctx)
            ...
            local node = ctx.cards[1]
            local cd = node and node.card_data
            if not cd or tonumber(cd.rank) ~= 6 then
                return
            end
            local hand = G and G.hand
            if not hand or not hand.destroy_card_node then
                return
            end
    ...
    j_hiker = {
        ...
        apply_effect = function(j, ctx)
            ...
            local node = ctx.card_node
            if not node or node.counts_for_play_score ~= true then
                return
            end
    ...
    j_gift = {
        ...
        apply_effect = function(j, ctx)
            ...
            local add = tonumber(j and j.effect_config and j.effect_config.extra)
                or tonumber(j and j.def and j.def.config and j.def.config.extra)
                or 1
    ...
    j_red_card = {
        matches_trigger = function(_, e)
            return e == "on_booster_skip" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_todo_list = {
        matches_trigger = function(_, e)
            return e == "on_hand_played" or e == "on_round_end"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_hand_played" then
    ...
    j_hallucination = {
        matches_trigger = function(_, e)
            return e == "on_booster_open"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name ~= "on_booster_open" then
                return
            end
    ...
    j_mail = {
        matches_trigger = function(_, e)
            return e == "on_discard" or e == "on_round_end"
        end,
        apply_effect = function(j, ctx)
    ...
    j_matador = {
        matches_trigger = function(_, e)
            return e == "on_boss_effect_triggered"
        end,
        apply_effect = function(_, ctx)
            add_money(ctx, 8)
        end,
    ...
    j_glass = {
        matches_trigger = function(_, e)
            return e == "on_hand_scored" or e == "glass_broken"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_hand_scored" then
    ...
    j_idol = {
        ...
        apply_effect = function(j, ctx)
            if ctx.event_name == "card_played" then
                if j.random_rank == nil or j.random_suit == nil then
                    return
                end
    ...
    j_card_sharp = {
        ...
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_hand_scored" then
                ...
                local played_count = (
                    G
                    and G.blind_hand_play_counts
                    and hand_idx
                    and G.blind_hand_play_counts[hand_idx]
                ) or 0
    ...
    j_madness = {
        matches_trigger = function(_, e)
            return e == "on_blind_selected" or e == "on_hand_scored"
        end,
        apply_effect = function(j, ctx)
    ...
    j_perkeo = {
        ...
        apply_effect = function(_, ctx)
            ...
            if not (G and G.add_consumable and type(G.consumables) == "table") then
                return
            end
            if #G.consumables < 1 then
                return
            end
            local src = G.consumables[math.random(1, #G.consumables)]
            if type(src) ~= "table" or type(src.id) ~= "string" or src.id == "" then
                return
            end
            local params = {}
            for k, v in pairs(src) do
    ...
    j_burnt = {
        matches_trigger = function(_, e)
            return e == "on_discard" or e == "on_round_begin" or e == "on_blind_selected"
        end,
        apply_effect = function(j, ctx)
            ...
            if ctx.event_name ~= "on_discard" or ctx.discard_reason ~= "discard" then
                return
            end
            if j._burnt_used_this_round == true then
                return
            end
            local hand_idx = tonumber(G and G.selectedHand)
            if not hand_idx or hand_idx < 1 then
                return
            end
    ...
    j_invisible = {
        matches_trigger = function(_, e)
            return e == "on_round_end" or e == "on_joker_sold"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_round_end" then
            ...
            if ctx.invisible_duplicated == true then
                return
            end
            if ctx.event_name ~= "on_joker_sold" or ctx.joker ~= j then
                return
            end
            if not (G and G.add_joker_by_def and type(G.jokers) == "table") then
                return
            end
            local required = math.max(1, math.floor(tonumber(j.def and j.def.config and j.def.config.extra) or 2))
            if (tonumber(j.runtime_counter) or 0) < required then
                return
            end
            if #G.jokers <= 0 then
                return
            ...
            if not (src and src.def and src.def.id) then
                return
            end
            ...
            if not G:add_joker_by_def(src.def.id, { edition = clone_edition }) then
                return
            end
    ...
    j_hit_the_road = {
        matches_trigger = function(_, e)
            return e == "on_discard" or e == "on_hand_scored" or e == "on_round_end"
        end,
        apply_effect = function(j, ctx)
            if ctx.event_name == "on_discard" and ctx.discard_reason == "discard" then
                local discarded = ctx.discarded_cards
...
function JokerEffects.should_shake_for_context(ctx)
    ...
    if ctx._suppress_joker_apply_shake then
        return false
    end
...
function JokerEffects.apply_shake_if_needed(joker, ctx)
    if not joker or not JokerEffects.should_shake_for_context(ctx) then
        return false
    end
