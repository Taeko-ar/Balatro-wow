...
local RUN_SAVE_DIR = "save"
local PROFILE_COUNT = 3
local ACTIVE_PROFILE_PATH = "save/Balatro_active_profile.lua"
local SETTINGS_SAVE_PATH_P1 = "save/Balatro_settings.lua"
local RUN_SAVE_PATH_P1 = "save/Balatro_run_save_1.lua"
...
function Game:get_most_played_hand_name()
    ...
    if not best_idx or best_count <= 0 then
        return "None"
    end
...
function Game:get_boss_blind_id_for_blind()
    ...
    if self:hasJoker("j_chicot") then
        return nil
    end
...
function Game:get_active_boss_blind_id()
    ...
    if not self:get_boss_blind_prototype() then
        return nil
    end
...
function Game:get_effective_hand_size_limit()
    ...
    for _, j in ipairs(self.jokers or {}) do
        ...
        if id == "j_juggler" then
            limit = limit + 1
        end
        if id == "j_turtle_bean" then
            limit = limit + j.runtime_counter
        end
        if id == "j_troubadour" then
            limit = limit + 2
        end
        if id == "j_stuntman" then
            limit = limit - 2
        end
        if id == "j_merry_andy" then
            limit = limit - 1
        end
    end
...
function Game:get_effective_hands_per_round()
    local hands = 4
    if self:has_voucher("v_hieroglyph") then
        hands = hands - 1
    end
    if self:has_voucher("v_grabber") then
        hands = hands + 1
    end
    if self:has_voucher("v_nacho") then
        hands = hands + 1
    end
    hands = hands + (self.deck_hands or 0)
    for _, j in ipairs(self.jokers or {}) do
        local id = j and j.def and j.def.id
        if id == "j_burglar" then
            hands = hands + 3
        end
        if id == "j_troubadour" then
            hands = hands - 1
...
function Game:get_effective_discards_per_round()
    local discards = 3
    if self:has_voucher("v_wasteful") then
        discards = discards + 1
    end
    if self:has_voucher("v_recyclomancy") then
        discards = discards + 1
    end
    if self:has_voucher("v_petroglyph") then
        discards = discards - 1
    end
    discards = discards + (self.deck_discards or 0) + (self._stake_discard or 0)
    for _, j in ipairs(self.jokers or {}) do
        local id = j and j.def and j.def.id
        if id == "j_drunkard" then
            discards = discards + 1
        end
        if id == "j_merry_andy" then
            discards = discards + 3
...
function Game:_boss_find_hand_node_by_uid(uid)
    if uid == nil or not self.hand or not self.hand.card_nodes then
        return nil
    end
    for _, node in ipairs(self.hand.card_nodes) do
        local d = node and node.card_data
        if d and d.uid == uid then
            return node
        end
    end
    return nil
...
function Game:_boss_select_forced_card_if_needed()
    if self:get_active_boss_blind_id() ~= "bl_final_bell" then
        return
    end
    if not self.hand or not self.hand.cards then
        return
    end
...
function Game:boss_on_hand_refilled(is_new_blind, refill_reason)
    local boss_id = self:get_active_boss_blind_id()
    if not boss_id or not self.hand or not self.hand.card_nodes then
        return
    end
...
function Game:boss_on_card_drawn(card_node)
    local boss_id = self:get_active_boss_blind_id()
    if not boss_id or not card_node then
        return
    end
    ...
    if boss_id == "bl_house" and (tonumber(self.boss_runtime.house_face_down_draws) or 0) > 0 then
        force_down = true
        self.boss_runtime.house_face_down_draws =
            math.max(0, (tonumber(self.boss_runtime.house_face_down_draws) or 0) - 1)
    end
    ...
    if boss_id == "bl_fish" and (tonumber(self.boss_runtime.fish_face_down_draws) or 0) > 0 then
        force_down = true
        self.boss_runtime.fish_face_down_draws =
            math.max(0, (tonumber(self.boss_runtime.fish_face_down_draws) or 0) - 1)
    end
...
function Game:boss_apply_on_hand_submitted(selected_nodes)
    ...
    if boss_id == "bl_tooth" then
        ...
        if (tonumber(self.money) or 0) < before then
            self:notify_boss_effect_triggered({
                reason = "tooth_money_loss",
                amount = before - (tonumber(self.money) or 0),
            })
        end
    ...
    elseif boss_id == "bl_arm" then
        ...
        if hs then
            ...
            self.selectedHandChips = (tonumber(hs.base_chips) or 0)
                + ((level - 1) * (tonumber(hs.chips_per_level) or 0))
            self.selectedHandMult = (tonumber(hs.base_mult) or 0) + ((level - 1) * (tonumber(hs.mult_per_level) or 0))
            if level < prev_level then
                self:notify_boss_effect_triggered({
                    reason = "arm_level_down",
...
function Game:boss_is_card_debuffed_for_scoring(node)
    if self.boss_runtime and self.boss_runtime.clear_card_debuffs_after_win == true then
        return false
    end
    local boss_id = self:get_active_boss_blind_id()
    if not boss_id or not node then
        return false
    end
    ...
    if boss_id == "bl_club" and (suit == "Clubs" or is_wild) then
        return true
    end
    if boss_id == "bl_goad" and (suit == "Spades" or is_wild) then
        return true
    end
    if boss_id == "bl_window" and (suit == "Diamonds" or is_wild) then
        return true
    end
    if boss_id == "bl_head" and (suit == "Hearts" or is_wild) then
        return true
    end
    if boss_id == "bl_plant" and ((rank >= 11 and rank <= 13) or self:hasJoker("j_pareidolia")) then
        return true
    end
    if boss_id == "bl_pillar" and d.uid and self._ante_played_card_uids[d.uid] then
        return true
    end
    if boss_id == "bl_final_leaf" and self.boss_runtime.verdant_leaf_active == true then
        return true
    end
    return false
end
...
function Game:boss_is_joker_debuffed(node)
    local boss_id = self:get_active_boss_blind_id()
    if boss_id ~= "bl_final_heart" or not node then
        return false
    end
    if type(self.jokers) ~= "table" then
        return false
            ...
            if need_joker and Joker then
                ...
                if type(def) == "table" then
                    ...
                    if type(create_params) ~= "table" then
                        create_params =
                            self:_build_joker_create_params(def, { edition = offer.edition }, offer.stickers)
                    end
                    if type(create_params) ~= "table" then
...
function Game:sync_shop_booster_nodes()
    if type(self.shop_booster_offers) ~= "table" then
        self.shop_booster_offers = {}
    end
    if type(self.shop_booster_nodes) ~= "table" then
        self.shop_booster_nodes = {}
    end
    if not ShopBoosterNode then
        return
    end
...
function Game:sync_shop_voucher_nodes()
    if type(self.shop_voucher_offers) ~= "table" then
        self.shop_voucher_offers = {}
    end
    if type(self.shop_voucher_nodes) ~= "table" then
        self.shop_voucher_nodes = {}
    end
    if not ShopVoucherNode then
        return
    end
...
function Game:can_buy_shop_offer(slot_index)
    ...
    if not self:can_afford_price(self:get_shop_offer_price(offer)) then
        return false
    end
    local k = offer.kind
...
function Game:get_drag_zones_for_context(ctx)
    if not ctx or not ctx.kind then
        return nil
    end
    local C = self.C or {}
    ...
    if ctx.kind == "shop_offer" then
        ...
        if offer and (offer.kind == "tarot" or offer.kind == "planet" or offer.kind == "spectral") then
            ...
            zones.top_right =
                DragZonesUI.make_zone("BUY and USE", can_buy_use, can_buy_use and C.GREEN or C.GREY, "buy_use", true)
        end
    ...
    elseif ctx.kind == "shop_voucher" then
        ...
        local can_buy = offer
            and self:can_afford_price(self:get_shop_voucher_price(offer))
    ...
    elseif ctx.kind == "owned_consumable" then
        ...
        zones.bottom = DragZonesUI.make_zone(string.format("Sell $%d", sell_value), true, C.MULT, "sell", true)
    elseif ctx.kind == "booster_choice" then
        ...
        local needs_use = ch
            and (ch.kind == "tarot" or ch.kind == "spectral")
            and c
            and (self:booster_tarot_needs_hand(c) or self:booster_spectral_needs_hand(c))
        if needs_use then
            local can_use = ch
                and not ch.taken
                and (tonumber(sess.picks_remaining) or 0) > 0
                and c
                and self:pack_consumable_can_apply(c)
...
function Game:perform_drag_zone_action(ctx, zone_id, zone)
    if not ctx or not zone_id or not zone or not zone.enabled then
        return false
    end
...
function Game:_inc_atlas_owner(name)
    if not self:_is_managed_joker_sprite_key(name) then
        return
    end
...
function Game:_register_joker_front_atlas_owner(joker)
    if not joker or joker._atlas_ref_registered == true then
        return
    end
...
function Game:on_joker_front_atlas_resolved(joker, old_name, new_name)
    ...
    if joker._atlas_ref_registered ~= true then
        return
    end
...
function Game:is_collection_discovery_id(id)
    ...
    if VOUCHER_DEFS and VOUCHER_DEFS[id] then
        return true
    end
    if self.P_TAGS and self.P_TAGS[id] then
        return true
    end
    if self.P_BLINDS and self.P_BLINDS[id] then
        return true
    end
    if id:sub(1, 12) == "enhancement_" then
        return true
    end
    if id:sub(1, 5) == "seal_" then
        return true
    end
    if id:sub(1, 8) == "edition_" then
        return true
    end
    if id:sub(1, 8) == "booster_" then
        return true
    end
    return false
end
...
function Game:is_trackable_discovery_id(id)
    ...
    if JOKER_DEFS and JOKER_DEFS[id] then
        return true
    end
...
function Game:discover_item(id)
    if not self:is_trackable_discovery_id(id) then
        return false
    end
    if not self.Discovered then
        self.Discovered = {}
    end
    if self.Discovered[id] == true then
        return false
    end
...
function Game:restore_run_deck_stake_from_snapshot(snapshot)
    ...
    if type(deck_id) == "string" and deck_id ~= "" then
        ...
        local def = DECK_DEFS_BY_ID and DECK_DEFS_BY_ID[deck_id]
        local cfg = def and def.config or {}
        ...
        if (tonumber(cfg.extra_hand_bonus) or 0) ~= 0 then
            self.extra_hand_bonus = tonumber(cfg.extra_hand_bonus)
        end
        if (tonumber(cfg.extra_discard_bonus) or 0) ~= 0 then
            self.extra_discard_bonus = tonumber(cfg.extra_discard_bonus)
        end
        if (tonumber(cfg.hand_size) or 0) ~= 0 then
            self.deck_hand_size = tonumber(cfg.hand_size)
        end
        if (tonumber(cfg.consumable_slots) or 0) ~= 0 then
            self.deck_consumable_slots = tonumber(cfg.consumable_slots)
        end
        if cfg.no_interest then
            self._deck_no_interest = true
        end
        if (tonumber(cfg.joker_slots) or 0) ~= 0 then
            self.deck_joker_slots = tonumber(cfg.joker_slots)
        ...
        if (tonumber(cfg.spectral_rate) or 0) ~= 0 then
            self.deck_spectral_rate = tonumber(cfg.spectral_rate)
        end
    end
...
function Game:ensure_victory_progress_recorded()
    if self._victory_progress_recorded == true then
        return true
    end
    if not self:get_run_stake_id() then
        return false
    end
...
function Game:record_stake_victory()
    ...
    if type(deck_id) ~= "string" or type(stake_id) ~= "string" then
        return false
    end
    if not self.unlocks then
        self:apply_unlocks(self:build_unlocks())
    ...
    for _, s in ipairs(STAKE_DEFS or {}) do
        if unlock_next then
            ...
            if next_stake then
                next_stake.unlocked = true
            end
            break
        end
        if s.id == stake_id then
            unlock_next = true
        end
    end
...
function Game:record_joker_wins_at_victory()
    local stake_id = self:get_run_stake_id()
    if type(stake_id) ~= "string" or stake_id == "" then
        return false
    end
    if not self.joker_wins then
        self:apply_joker_wins(self:build_joker_wins())
    end
...
function Game:build_pause_focus_targets()
    ...
    if self._pause_show_settings then
        ...
        for i, r in ipairs(self._pause_speed_rects or {}) do
            if r then
                targets[#targets + 1] = { kind = "speed", index = i, rect = r }
            end
        end
...
function Game:settings_path_for_profile(profile_id)
    ...
    if id <= 1 then
        return SETTINGS_SAVE_PATH_P1
    end
    return string.format("save/Balatro_settings_%d.lua", id)
end

function Game:run_save_path_for_profile(profile_id)
    local id = math.floor(tonumber(profile_id) or 1)
    if id <= 1 then
        return RUN_SAVE_PATH_P1
    end
    return string.format("save/Balatro_run_save_%d.lua", id)
end
...
function Game:snapshot_settings()
    return {
        ...
        GRAPHICS = {
            texture_scaling = tonumber(
                self.SETTINGS and self.SETTINGS.GRAPHICS and self.SETTINGS.GRAPHICS.texture_scaling
            ) or 1,
        },
...
function Game:set_role_held(role, held, press_time)
    if not InputBindings.HOLD_ROLES[role] then
        return
    end
    if type(self._role_held) ~= "table" then
        self._role_held = {}
    end
    if type(self._role_press_time) ~= "table" then
        self._role_press_time = {}
    end
    if held then
...
function Game:set_game_speed(speed)
    ...
    if not s or not allowed_speeds[s] then
        return
    end
...
function Game:can_open_deck_view()
    ...
    if self.STATE == self.STATES.MENU or self.STATE == self.STATES.GAME_OVER or self.STATE == self.STATES.YOU_WIN then
        return false
    end
    if self.STATE == self.STATES.PAUSED then
        return false
...
function Game:enter_deck_view()
    if self._deck_view_open or not self:can_open_deck_view() then
        return false
    end
    self.dragging = nil
    self.active_tooltip_card = nil
...
function Game:exit_deck_view()
    if not self._deck_view_open then
        return false
    end
    self.dragging = nil
    self.active_tooltip_card = nil
...
function Game:clear_run_snapshot()
    ...
    if not self:has_saved_run() then
        return true
    end
        ...
        if ok then
            ...
            if j then
                ...
                if jrec.random_suit ~= nil or jrec.random_rank ~= nil or jrec.random_hand ~= nil then
                    local restore = {}
                    if jrec.random_suit ~= nil then
                        restore.random_suit = jrec.random_suit
                    end
                    if jrec.random_rank ~= nil then
                        restore.random_rank = jrec.random_rank
                    end
                    if jrec.random_hand ~= nil then
                        restore.random_hand = jrec.random_hand
                    end
...
function Game:mark_boss_used(boss_id)
    if type(boss_id) ~= "string" or boss_id == "" then
        return
    end
...
function Game:get_boss_effect_text()
    ...
    if not boss_id then
        ...
        if p then
            boss_id = self.current_boss_blind_id
        end
    end
...
function Game:get_blind_color(index)
    local def = self:get_blind_def(index)
    if not def then
        return self.C.BLIND_COLORS.Big
    end
    if def.id == "boss" then
        local proto = self:get_boss_blind_prototype()
...
function Game:init_item_prototypes()
    self.P_TAGS = {
        ...
        tag_rare = {
            ...
            min_ante = nil,
            order = 2,
            config = { type = "store_joker_create", odds = 3 },
        ...
        tag_negative = {
            ...
            min_ante = 2,
            order = 3,
            config = { type = "store_joker_modify", edition = "negative", odds = 5 },
            requires = "e_negative",
            pos = { x = 2, y = 0 },
        },
        tag_foil = {
            ...
            min_ante = nil,
            order = 4,
            config = { type = "store_joker_modify", edition = "foil", odds = 2 },
            requires = "e_foil",
            pos = { x = 3, y = 0 },
        },
        tag_holo = {
            ...
            min_ante = nil,
            order = 5,
            config = { type = "store_joker_modify", edition = "holo", odds = 3 },
            requires = "e_holo",
            pos = { x = 0, y = 1 },
        },
        tag_polychrome = {
            ...
            min_ante = nil,
            order = 6,
            config = { type = "store_joker_modify", edition = "polychrome", odds = 4 },
        ...
        tag_garbage = {
            ...
            min_ante = 2,
            order = 15,
            config = { type = "immediate", dollars_per_discard = 1 },
            pos = { x = 2, y = 3 },
        },
        tag_ethereal = {
            name = "Ethereal Tag",
        ...
        tag_skip = {
            ...
            min_ante = nil,
            order = 22,
            config = { type = "immediate", skip_bonus = 5 },
            pos = { x = 0, y = 3 },
        },
        tag_orbital = {
            name = "Orbital Tag",
            ...
            min_ante = 2,
            order = 23,
            config = { type = "immediate", levels = 3 },
            pos = { x = 5, y = 2 },
        },
        tag_economy = {
            name = "Economy Tag",
...
function Game:draw()
    ...
    local show_consumables = not (
        self.STATE == self.STATES.ROUND_EVAL
        or self.STATE == self.STATES.GAME_OVER
        or self.STATE == self.STATES.YOU_WIN
    )
    ...
    for _, node in ipairs(self.nodes) do
        if
            not node._deck_view_card
            and not node._collection_node
            and not cons_set[node]
            and not hand_set[node]
            and not joker_set[node]
        then
            node:draw()
        end
    end
...
function Game:draw_tooltips_on_top()
    ...
    if self.nodes then
        for _, node in ipairs(self.nodes) do
            ...
            local is_booster_cons = is_booster
                and Consumable
                and node
                and node.is
                and node:is(Consumable)
                and node._booster_choice_index
            ...
            if
                ...
                and node:is(Card)
                and node._booster_choice_index
                and node.draw_tooltip_overlay
            then
                node:draw_tooltip_overlay()
            end
            local is_shop_pc = (self.STATE == self.STATES.SHOP)
                and Card
                and node
                and node.is
                and node:is(Card)
                and node.shop_offer_slot
...
function Game:_draw_shop_voucher_tooltip()
    ...
    if type(offer) ~= "table" or type(rect) ~= "table" then
        return
    end
...
function Game:add_consumable(def_id, create_params)
    ...
    if not CONSUMABLE_DEFS or type(CONSUMABLE_DEFS) ~= "table" then
        return false
    end
    ...
    if not self.consumable_nodes then
        self.consumable_nodes = {}
    end
    ...
    if not self:can_add_consumable(params) then
        return false
    end
...
function Game:remove_consumable_at(index)
    ...
    if not self.consumables or type(self.consumables) ~= "table" then
        return nil
    end
    ...
    if self.active_tooltip_consumable_index and self.active_tooltip_consumable_index >= index then
        self.active_tooltip_consumable_index = math.min(#self.consumables, self.active_tooltip_consumable_index)
    end
...
function Game:hand_ready_for_tarot_selection()
    ...
    if self.hand.is_scoring_active and self.hand:is_scoring_active() then
        return false
    end
    return true
end
...
function Game:ordered_selected_hand_count()
    if not self.hand or not self.hand.ordered_selected_nodes then
        return 0
    end
...
function Game:tarot_selection_requirement_met(c)
    ...
    if smin and n < smin then
        return false
    end
    if smax and n > smax then
        return false
    end
    return true
end
...
function Game:consumable_use_enabled(idx)
    if not self:consumable_play_state_ok() then
        return false
    end
    ...
    if c.kind == "tarot" then
        ...
        local spawns = c.spawn
            or c.spawn_joker
            or c.wheel_of_fortune
            or c.fool_duplicate
            or c.hermit_money
            or c.temperance_money
        if not spawns and need_hand and not self:hand_ready_for_tarot_selection() then
            return false
        end
        ...
        if c.spawn then
            if self:consumable_slots_after_use_index(idx) < 1 then
                return false
            end
        end
        if c.fool_duplicate then
            local last = self.last_consumable_use_id
            if not last or last == "tarot_fool" then
                return false
            end
            if not CONSUMABLE_DEFS or not CONSUMABLE_DEFS[last] then
                return false
            end
            if self:consumable_slots_after_use_index(idx) < 1 then
                return false
        ...
        if c.spawn_joker and not self:joker_has_room_for_new("base") then
            return false
        end
        if c.wheel_of_fortune then
            if not self.jokers or #self.jokers < 1 then
                return false
            end
        end
        return true
    ...
    if c.kind == "spectral" then
        ...
        if sid == "spectral_wraith" or sid == "spectral_soul" then
            if not self:joker_has_room_for_new("base") then
                return false
            end
        end
        ...
        if need_hand then
            if not self:hand_ready_for_tarot_selection() then
                return false
            end
...
function Game:shop_offer_consumable_use_enabled(offer)
    ...
    if kind ~= "tarot" and kind ~= "planet" and kind ~= "spectral" then
        return false
    end
    ...
    if self:consumable_requires_selection(def) then
        return false
    end
...
function Game:has_played_hand_name(hand_name)
    ...
    if type(self.handlist) ~= "table" then
        return false
    end
    local idx = nil
...
function Game:is_joker_pool_swap_active(from_id, to_id)
    return self.joker_pool_replacements and self.joker_pool_replacements[from_id] == to_id
end
...
function Game:deck_has_enhancement(enhancement)
    if type(enhancement) ~= "string" or enhancement == "" then
        return false
    end
    return self:count_cards_in_full_deck(function(c)
        return c.enhancement == enhancement
    end) > 0
end

function Game:joker_meets_deck_requirement(joker_id)
    if type(joker_id) ~= "string" then
        return true
    end
...
function Game:joker_allowed_in_random_pool(joker_id)
    ...
    if not self:joker_meets_deck_requirement(joker_id) then
        return false
    end
    for from_id, to_id in pairs(self.joker_pool_replacements or {}) do
        if joker_id == from_id then
            return false
        end
        if joker_id == to_id then
            return true
        end
    end
    for _, pair in ipairs(self.joker_pool_swap_pairs or {}) do
        if joker_id == pair.to then
            return false
        end
    end
    return true
...
function Game:replace_shop_joker_offer(offer, from_id, to_id)
    ...
    if offer.kind ~= "joker" and offer.kind ~= nil then
        return offer
    end
    if offer.id ~= from_id then
        return offer
    end
...
function Game:purge_joker_pool_swap_from_shop(from_id, to_id)
    if not self:is_joker_pool_swap_active(from_id, to_id) then
        return
    end
...
function Game:random_joker_def_id_by_rarity(rarity)
    ...
    if not r or not JOKER_DEFS then
        return nil
    end
...
function Game:upgrade_hand_level_at_index(idx)
    idx = tonumber(idx)
    if not idx or not self.hand_stats or not self.hand_stats[idx] then
        return false
    end
...
function Game:apply_consumable_effect(c)
    ...
    if kind == "spectral" then
        ...
        local function add_generated_card(rank, suit, enhancement)
            if not hand or not hand.add_card then
                return nil
            end
            local cd = {
                rank = rank,
        ...
        elseif id == "spectral_talisman" then
            if ord[1] and ord[1].set_seal then
                ord[1]:set_seal("gold")
            end
        ...
        elseif id == "spectral_ankh" then
            if self.jokers and #self.jokers > 0 then
                ...
                if src_id and src then
                    ...
                    if
                        self:joker_has_room_for_new(src_edition)
                        and self:add_joker_by_def(src_id, { edition = src_edition })
                    then
        ...
        elseif id == "spectral_hex" then
            if self.jokers and #self.jokers > 0 then
                ...
                if target and Joker then
                    ...
                    if target.refresh_quads then
                        target:refresh_quads()
                    end
                end
                for i = #self.jokers, 1, -1 do
                    if i ~= keep then
    ...
    if id == "tarot_fool" then
        ...
        if last_id and last_id ~= "tarot_fool" then
            ...
            if Sfx and Sfx.play_money then
                Sfx.play_money()
            end
        end
        clear_tarot_hand_ui()
    ...
    elseif id == "tarot_lovers" then
        if ord[1] then
            ord[1]:set_enhancement("wild")
        end
    elseif id == "tarot_chariot" then
        if ord[1] then
            ord[1]:set_enhancement("steel")
        end
    elseif id == "tarot_justice" then
        if ord[1] then
            ord[1]:set_enhancement("glass")
        end
    ...
    elseif id == "tarot_devil" then
        if ord[1] then
            ord[1]:set_enhancement("gold")
        end
    elseif id == "tarot_tower" then
        if ord[1] then
            ord[1]:set_enhancement("stone")
        end
    ...
    if id ~= "tarot_hanged_man" then
        ...
        self.active_tooltip_card = nil
        if hand and hand.calculate_play then
            hand:calculate_play()
        end
    end

    local sfx_mult = id ~= "tarot_hermit"
        and id ~= "tarot_temperance"
        and id ~= "tarot_wheel_of_fortune"
        and id ~= "tarot_high_priestess"
        and id ~= "tarot_emperor"
        and id ~= "tarot_judgement"
...
function Game:use_consumable(index)
    if not self:consumable_use_enabled(index) then
        return false
    end
...
function Game:draw_consumables_row()
    self._consumable_rects = {}
    if not self.consumables or #self.consumables == 0 then
        return
    end
    self:_apply_consumable_layout()
end
...
function Game:try_reorder_consumable_after_drag(consumable_node, release_x)
    if not consumable_node or not self.consumable_nodes or self.consumables_on_bottom ~= true then
        return false
    end
...
function Game:ensure_skip_orbital_hand(blind_index)
    ...
    if not blind_index or not self.skips or self.skips[blind_index] ~= 17 then
        return
    end
...
function Game:draw_skip_tag_icon(tag_type, x, y, scale)
    if type(tag_type) ~= "string" or tag_type == "" or not Tag then
        return
    end
    love.graphics.push()
...
function Game:skip_blind(index)
    ...
    if not blind_index or blind_index < 1 then
        return false
    end
...
function Game:draw_bottom_blind_select()
    ...
    for i = 1, 3 do
        ...
        love.graphics.print(
            "$" .. string.rep("$", reward) .. "+",
            rx + love.graphics.getFont():getWidth("Reward: "),
            ty + 24
        )
        ...
        if def and def.id ~= "boss" then
            ...
            love.graphics.print(
                buttonText,
                buttonX + math.floor(buttonW / 2) - math.floor(love.graphics.getFont():getWidth(buttonText) / 2),
                buttonY + math.floor(buttonH / 2) - math.floor(love.graphics.getFont():getHeight(buttonText) / 2)
            )
...
function Game:draw_bottom_pause()
    ...
    local function is_pause_focused(kind, index)
        ...
        for i, t in ipairs(targets) do
            if i == pause_focus and t.kind == kind then
                if index == nil or t.index == index then
                    return true
                end
            end
        end
    ...
    if self._pause_show_settings then
        ...
        draw_btn(
            self._pause_save_quit_rect,
            "Save and Quit",
            can_save and self.C.BLUE or self.C.GREY,
            is_pause_focused("save_quit")
        )
...
function Game:_compute_fanned_joker_row(n, screen_w, card_w, gap_w, margin)
    ...
    if n <= 0 then
        return 0, 0, math.floor(screen_w * 0.5 + 0.5)
    end
...
function Game:recompute_consumable_slot_layout()
    ...
    self.consumable_slot_start_x_bottom = (self.jokers_on_bottom == true) and (bot_dims.consumable_panel_x + rel_bot)
        or rel_bot
...
function Game:get_joker_shared_pick(def_id)
    if not self:uses_joker_shared_picks(def_id) then
        return nil
    end
...
function Game:set_joker_shared_picks(def_id, picks)
    if not self:uses_joker_shared_picks(def_id) or type(picks) ~= "table" then
        return
    end
    self.joker_shared_picks = self.joker_shared_picks or {}
...
function Game:add_joker_by_def(def_id, create_params)
    ...
    if not JOKER_DEFS or type(JOKER_DEFS) ~= "table" then
        return false
    end
    ...
    if not self.joker_capacity then
        self.joker_capacity = self:joker_base_capacity()
    end
    if not self.jokers then
        self.jokers = {}
...
function Game:count_jokers_with_id(joker_id)
    if type(self.jokers) ~= "table" or type(joker_id) ~= "string" or joker_id == "" then
        return 0
    end
    local n = 0
    for _, j in ipairs(self.jokers) do
...
function Game:_apply_joker_layout()
    ...
    if self.jokers_on_bottom == true then
        ...
        local step, total_span, start_x = self:_compute_fanned_joker_row(n, panel_w, eff_w, eff_gap, 8)
...
function Game:_apply_consumable_layout()
    ...
    if self.consumables_on_bottom == true then
        ...
        local step, total_span, rel_start = self:_compute_fanned_joker_row(n, panel_w, eff_w, eff_gap, 8)
        local start_x = (self.jokers_on_bottom == true) and (bot_dims.consumable_panel_x + rel_start) or rel_start
...
function Game:_collect_jokers_in_slot_order()
    local out = {}
    if not self.jokers or type(self.jokers) ~= "table" then
        return out
    end
    for _, j in ipairs(self.jokers) do
        if j then
...
function Game:sum_retrigger_extras(held, retrigger_ctx)
    if type(retrigger_ctx) ~= "table" then
        return 0
    end
...
function Game:emit_hand_cards_event(event_name, ctx)
    ...
    if not self.hand or not self.hand.card_nodes then
        return
    end
...
function Game:emit_on_destroy_cards(destroyed_cards)
    if type(destroyed_cards) ~= "table" or #destroyed_cards == 0 then
        return
    end
...
function Game:_apply_observatory_voucher_to_hand_scored_ctx(ctx)
    if not self:has_voucher("v_observatory") then
        return
    end
    if type(ctx) ~= "table" or type(self.consumables) ~= "table" or not CONSUMABLE_DEFS then
        return
    end
    local hand_type = ctx.hand_type
    if type(hand_type) ~= "string" then
        return
    end
...
function Game:_apply_one_joker_emit()
    ...
    if j then
        ...
        if j.apply_effect then
            if
                q.pre_matched == true or (j.matches_trigger and q.event_name and j:matches_trigger(q.event_name, q.ctx))
            then
...
function Game:_update_joker_emit_queue(dt)
    if not self._joker_emit_queue then
        return
    end
...
function Game:has_voucher(voucher_id)
    if type(voucher_id) ~= "string" or voucher_id == "" then
        return false
    end
    local vs = self.vouchers
    if type(vs) == "table" then
        if vs[voucher_id] == true then
            return true
        end
        for _, v in ipairs(vs) do
            if v == voucher_id then
                return true
            end
        end
    end
...
function Game:get_shop_discount_multiplier()
    if self:has_voucher("v_liquidation") then
        return 0.5
    end
    if self:has_voucher("v_clearance_sale") then
        return 0.75
    end
    return 1
end
...
function Game:_roll_shop_playing_card_offer()
    if not self:has_voucher("v_magic_trick") then
        return nil
    end
    local suits = { "Hearts", "Clubs", "Diamonds", "Spades" }
...
function Game:_shop_voucher_candidate_ids(exclude_ids)
    ...
    for vid, def in pairs(VOUCHER_DEFS) do
        if type(def) == "table" and type(vid) == "string" and not exclude_ids[vid] then
            if not (self:_voucher_already_owned(vid)) then
...
function Game:apply_voucher_tags_to_shop()
    ...
    for _, i in ipairs(to_remove) do
        ...
        if offer then
            if not self.shop_voucher_offers then
                self.shop_voucher_offers = {}
            end
...
function Game:buy_shop_voucher(slot_index)
    if type(slot_index) ~= "number" or slot_index < 1 then
        return false
    end
    if self.STATE ~= self.STATES.SHOP then
        return false
    ...
    if type(offer) ~= "table" or type(offer.id) ~= "string" then
        return false
    end
    if not self:can_afford_price(self:get_shop_voucher_price(offer)) then
        return false
    end
    if self:_voucher_already_owned(offer.id) then
        return false
    end

    local voucher_id = offer.id
    self.money = (tonumber(self.money) or 0) - self:get_shop_voucher_price(offer)
    if not self.vouchers then
        self.vouchers = {}
    end
...
function Game:apply_voucher_effect(id)
    if id == "v_wasteful" or id == "v_recyclomancy" then
        return
    end
    if id == "v_tarot_merchant" or id == "v_tarot_tycoon" then
        return
    end
    if id == "v_planet_merchant" or id == "v_planet_tycoon" then
        return
    end
    if id == "v_seed_money" or id == "v_money_tree" then
        return
    end
    if id == "v_blank" then
        return
    ...
    if id == "v_magic_trick" or id == "v_illusion" then
        return
    end
    ...
    if id == "v_directors_cut" or id == "v_retcon" then
        return
    end
    ...
    if id == "v_clearance_sale" or id == "v_liquidation" then
        return
    end
    if id == "v_hone" or id == "v_glow_up" then
        return
    end
    if id == "v_reroll" or id == "v_reroll_glut" then
        return
    end
    ...
    if id == "v_omen_globe" then
        return
    end
    if id == "v_telescope" or id == "v_observatory" then
        return
    end
end

function Game:get_interest_round_cap_dollars()
    if self:has_voucher("v_money_tree") then
        return 20
    end
    if self:has_voucher("v_seed_money") then
        return 10
    end
    return 5
end
...
function Game:_deck_inject_playing_card(card_data)
    if not Deck or not Deck.copy_card_data then
        return false
    end
    ...
    local deck = self.deck
    if not deck or type(deck.cards) ~= "table" then
        return false
    end
    local n = #deck.cards
...
function Game:try_boss_reroll_press(x, y)
    ...
    if not r or not self:_point_in_rect_simple(x, y, r) then
        return false
    end
    if not (self:has_voucher("v_directors_cut") or self:has_voucher("v_retcon")) then
        return true
    end
    if tonumber(self.selected_blind_index) ~= 3 then
        return true
    end
    if not self:can_afford_price(10) then
        return true
    end
    if self:has_voucher("v_directors_cut") and not self:has_voucher("v_retcon") then
        if (tonumber(self.boss_rerolls_used_this_ante) or 0) >= 1 then
            return true
        end
    end
...
function Game:_pop_shop_queue_entry()
    ...
    while guard < 64 do
        guard = guard + 1
        if #(self.shop_offer_queue or {}) == 0 then
            break
        end
        ...
        if not (is_joker and type(entry.id) == "string" and not self:joker_meets_deck_requirement(entry.id)) then
            return entry
        end
    end
...
function Game:_pick_joker_id_shop_rarity_distribution(rand_int)
    ...
    local candidates = {}
    local allow_dup = self:hasJoker("j_ring_master")
    local function owned(id)
        return (not allow_dup) and self:_shop_joker_owned(id)
    end
    for id, def in pairs(JOKER_DEFS) do
        if type(def) == "table" and type(id) == "string" then
            local rv = tonumber(def.rarity) or 1
            if rv == target_rar and rv >= 1 and rv <= 3 and self:joker_allowed_in_random_pool(id) and not owned(id) then
    ...
    if #candidates == 0 then
        for id, def in pairs(JOKER_DEFS) do
            if type(def) == "table" and type(id) == "string" then
                local rv = tonumber(def.rarity) or 1
                if rv >= 1 and rv <= 3 and self:joker_allowed_in_random_pool(id) and not owned(id) then
                    candidates[#candidates + 1] = id
                end
            end
        end
...
function Game:_roll_shop_queue_joker_offer()
    ...
    if sticker_params.eternal then
        offer.eternal = true
    end
    if sticker_params.perishable then
        offer.perishable = true
    end
    if sticker_params.rental then
        offer.rental = true
    end
    return offer
end
...
function Game:_roll_shop_queue_consumable_offer(wanted_kind)
    if type(CONSUMABLE_DEFS) ~= "table" then
        return nil
    end
    local ids = {}
    for id, def in pairs(CONSUMABLE_DEFS) do
...
function Game:shop_price_for_joker_offer(def, edition, sticker_params)
    ...
    if
        (sticker_data and sticker_data.rental == true)
        or def.rental == true
        or (type(def.stickers) == "table" and def.stickers.rental == true)
    then
        return 1
    end
...
function Game:_build_joker_sticker_params(def)
    ...
    if type(def) == "table" then
        ...
        if def.eternal == true then
            params.eternal = true
        end
        if def.perishable == true then
            params.perishable = true
        end
        if def.rental == true then
            params.rental = true
        end
    end
...
function Game:extend_shop_offers_to_slot_count()
    ...
    while #self.shop_offers < slots and guard < guard_limit do
            ...
            if not dup then
                ...
                if ed == "base" then
                    ...
                    if edition_from_tag then
                        ...
                        entry.create_params =
                            self:_build_joker_create_params(def, { edition = edition_from_tag }, entry.stickers)
                ...
                if self:hasTag("coupon") ~= -1 and self.shop_reroll_count == 0 then
                    entry.price = 0
                end
        ...
        elseif entry.kind == "playing_card" then
            ...
            if not ((not allow_duplicates) and self:_shop_consumable_owned(entry.id)) then
                if self:hasTag("coupon") ~= -1 and self.shop_reroll_count == 0 then
                    entry.price = 0
                end
                self.shop_offers[#self.shop_offers + 1] = entry
...
function Game:shop_current_reroll_cost()
    ...
    if self.shop_reroll_count == 0 and self:hasJoker("j_chaos") then
        return 0
    end
    local sub = 0
    if self:has_voucher("v_reroll_glut") then
        sub = sub + 2
    end
    if self:has_voucher("v_reroll") then
        sub = sub + 2
    end
    if self._shop_reroll_base_cost_override ~= nil then
...
function Game:roll_skips()
    ...
    local function tag_key_to_id(tag_key)
        if type(tag_key) ~= "string" then
            return -1
        end
        local type_name = tag_key:match("^tag_(.+)$")
        if type_name == "uncommon" then
            return 0
        end
        if type_name == "rare" then
            return 1
        end
        if type_name == "negative" then
            return 2
        end
        if type_name == "foil" then
            return 3
        end
        if type_name == "coupon" then
            return 4
        end
        if type_name == "double" then
            return 5
        end
        if type_name == "holo" then
            return 6
        end
        if type_name == "polychrome" then
            return 7
        end
        if type_name == "investment" then
            return 8
        end
        if type_name == "voucher" then
            return 9
        end
        if type_name == "topup" then
            return 10
        end
        if type_name == "juggle" then
            return 11
        end
        if type_name == "boss" then
            return 12
        end
        if type_name == "standard" then
            return 13
        end
        if type_name == "charm" then
            return 14
        end
        if type_name == "meteor" then
            return 15
        end
        if type_name == "buffoon" then
            return 16
        end
        if type_name == "orbital" then
            return 17
        end
        if type_name == "speed" then
            return 18
        end
        if type_name == "handy" then
            return 19
        end
        if type_name == "garbage" then
            return 20
        end
        if type_name == "ethereal" then
            return 21
        end
        if type_name == "economy" then
            return 22
        end
        if type_name == "d6" then
            return 23
...
function Game:buy_shop_booster(slot_index)
    ...
    if not offer or offer.kind ~= "booster" then
        return false
    end
    local price = self:get_shop_booster_price(offer)
    if not self:can_afford_price(price) then
        return false
    end
...
function Game:_booster_destroy_choice_nodes()
    local sess = self.booster_session
    if not sess or type(sess.choice_nodes) ~= "table" then
        return
    end
...
function Game:_shop_pick_unique_joker_ids(count)
    ...
    local function joker_already_picked(id)
        ...
        if not allow_duplicates and self:_shop_joker_owned(id) then
            return true
        end
...
function Game:_booster_build_choices(offer)
        ...
        for _, e in ipairs(entries) do
            if e and e.id then
                ...
                choices[#choices + 1] = {
                    ...
                    taken = false,
                    create_params = self:_build_joker_create_params(
                        JOKER_DEFS and JOKER_DEFS[e.id],
                        { edition = e.edition or "base" },
                        sticker_params
                    ),
                    stickers = sticker_params,
    ...
    elseif pack == "standard" then
        local suits = { "Hearts", "Clubs", "Diamonds", "Spades" }
        local enh = { "bonus", "mult", "wild", "glass", "steel", "stone", "gold", "lucky" }
        local edition_rate = (self:has_voucher("v_glow_up") and 4) or (self:has_voucher("v_hone") and 2) or 1
        for _ = 1, n do
            local rank = self:_shop_rand_int(2, 14)
            local suit = suits[self:_shop_rand_int(1, #suits)]
            local data = { rank = rank, suit = suit, enhancement = nil, seal = nil }
            if math.random() > 0.6 then
                data.enhancement = enh[math.random(1, #enh)]
            end
            local ep = math.random()
            local ed = (ep > 1 - 0.006 * edition_rate * 2 and "polychrome")
                or (ep > 1 - 0.02 * edition_rate * 2 and "holo")
                or (ep > 1 - 0.04 * edition_rate * 2 and "foil")
                or nil
            if ed then
                data.modifier =
                    { edition = ed, chip_bonus = ed == "foil" and 50 or 0, mult_bonus = ed == "holo" and 10 or 0 }
            end
            if math.random() > 1 - 0.02 * 10 then
                local st = math.random()
                data.seal = (st > 0.75 and "red") or (st > 0.5 and "blue") or (st > 0.25 and "gold") or "purple"
            end
            choices[#choices + 1] = { kind = "playing", playing_data = data, taken = false }
        end
    end
    ...
    for i, ch in ipairs(choices) do
        ...
        elseif ch.kind == "joker" and Joker then
            ...
            if type(jd) == "table" then
                local create_params = type(ch.create_params) == "table" and copy_table(ch.create_params)
...
function Game:_booster_discard_pack_hand_maybe_refill()
    local sess = self.booster_session
    if not sess or not sess.hand_for_tarot then
        return
    end
...
function Game:booster_tarot_needs_hand(c)
    if type(c) ~= "table" or c.kind ~= "tarot" then
        return false
    end
    local need_hand = false
    local s = c.select
...
function Game:booster_spectral_needs_hand(c)
    if type(c) ~= "table" or c.kind ~= "spectral" then
        return false
    end
    local s = c.select
    if type(s) ~= "table" then
    ...
    if (s.exact or 0) > 0 then
        return true
    end
    if s.min and tonumber(s.min) > 0 then
        return true
    end
    return false
end
...
function Game:pack_consumable_can_apply(c)
    ...
    if c.fool_duplicate then
        ...
        local cap = self:get_effective_consumable_capacity()
        if math.max(0, cap - #(self.consumables or {})) < 1 then
            return false
        end
    end
    if c.spawn_joker and not self:joker_has_room_for_new("base") then
...
function Game:pick_booster_choice(idx)
    local sess = self.booster_session
    if not sess or type(sess.choices) ~= "table" then
        return false
    end
    local ch = sess.choices[idx]
    if not ch or ch.taken then
        return false
    end
    ...
    if ch.kind == "planet" then
        ...
        if not self:pack_consumable_can_apply(c) then
            return false
        end
        self:track_consumable_use(c)
        self:apply_consumable_effect(c)
    elseif ch.kind == "joker" then
        ...
        if not self:joker_has_room_for_new(ed) then
            return false
        end
        local create_params = type(ch.create_params) == "table" and copy_table(ch.create_params)
            or ((ed ~= "base") and { edition = ed } or nil)
    ...
    if sess.picks_remaining <= 0 then
...
function Game:enter_round_win_after_blind()
    Sfx.play("Assets/sounds/win.ogg")
    if tonumber(self.current_blind_index) == 3 then
        self.boss_runtime = self.boss_runtime or {}
    ...
    if self._deck_no_interest then
        interest = 0
    end
    ...
    if k == nil or k == "joker" then
        ...
        if type(def) == "table" and type(create_params) ~= "table" then
            ...
            if next(sticker_flags) ~= nil then
                ...
                for flag, v in pairs(sticker_flags) do
                    create_params[flag] = v
                end
            end
        end
...
function Game:buy_and_use_shop_consumable(slot_index)
    ...
    if not self:shop_offer_consumable_use_enabled(offer) then
        return false
    end
    local def = CONSUMABLE_DEFS and CONSUMABLE_DEFS[offer.id]
    if type(def) ~= "table" then
...
function Game:sell_owned_joker(index)
    ...
    if joker and joker.def and joker.def.id == "j_invisible" then
        ...
        local required =
            math.max(1, math.floor(tonumber(joker.def and joker.def.config and joker.def.config.extra) or 2))
    ...
    if invisible_ready and type(self.jokers) == "table" and #self.jokers > 0 then
        ...
        if src and src.def and src.def.id and self.add_joker_by_def then
            local src_edition = Joker and Joker.normalize_edition and Joker.normalize_edition(src.edition)
                or tostring(src.edition or "base")
...
function Game:handle_failed_blind_reset()
    ...
    if mr_bones_index then
        ...
        if Sfx and Sfx.play then
            Sfx.play("Assets/sounds/slice1.ogg")
        end
        self._blind_resolution_pending = false
        self:recycle_full_deck_after_blind_win()
    ...
    if Sfx and Sfx.play then
        Sfx.play("Assets/sounds/cancel.ogg")
    end
    self:clear_run_snapshot()
...
function Game:refocus_after_bottom_panel_closed(closed_layer)
    if self:get_gamepad_focus_layer() ~= closed_layer then
        return
    end
...
function Game:set_jokers_location(on_bottom)
    if on_bottom == true and #(self.jokers or {}) == 0 then
        return
    end
    if self.jokers_on_bottom == (on_bottom == true) then
        return
    end
...
function Game:set_consumables_location(on_bottom)
    if on_bottom == true and #(self.consumables or {}) == 0 then
        return
    end
    if self.consumables_on_bottom == (on_bottom == true) then
        return
    end
    local to_bottom = on_bottom == true
...
function Game:try_reorder_joker_after_drag(joker_node, release_x)
    if not joker_node or not self.jokers or not self.jokers_on_bottom then
        return false
    end
...
local function node_is_hand_card(self, node)
    if not node or not self or not self.hand or not self.hand.card_nodes then
        return false
    end
    for _, hn in ipairs(self.hand.card_nodes) do
        if hn == node then
...
local function node_is_owned_joker(self, node)
    if not node or not self or not self.jokers then
        return false
    end
    for _, j in ipairs(self.jokers) do
        if j == node then
...
local function node_is_shop_offer(self, node)
    if not node or not self or not self.shop_offer_nodes then
        return false
    end
...
local function node_is_owned_consumable(self, node)
    if not node or not self or not self.consumable_nodes then
        return false
    end
...
local function node_is_zone_draggable(self, node)
    ...
    if node_is_shop_offer(self, node) then
        return true
    end
    if node_is_shop_booster_node(node) then
        return true
    end
    if node_is_shop_voucher_node(node) then
        return true
    end
    if node_is_booster_choice(node) then
        return true
    end
    if node_is_owned_joker(self, node) then
        return true
    end
    if node_is_owned_consumable(self, node) then
        return true
    end
    return false
end
...
function Game:touchpressed(id, x, y)
    ...
    if self.STATE == self.STATES.PAUSED then
        if self._pause_show_settings then
            if self._pause_settings_tab == "controls" then
                ...
                if
                    self._pause_controls_reset_rect
                    and self:_point_in_rect_simple(x, y, self._pause_controls_reset_rect)
                then
            ...
            if slider and self:_point_in_rect_simple(x, y, slider) then
                ...
                if vol ~= nil then
                    self:set_music_volume(vol, { skip_save = true })
                end
                return
            end
    ...
    if self.STATE == self.STATES.BLIND_SELECT then
        ...
        if self:handle_blind_select_touch(x, y) then
            return
        end
    end
    if self.STATE == self.STATES.ROUND_EVAL then
        ...
        if self:handle_round_win_touch(x, y) then
            return
        end
    end
    if self.STATE == self.STATES.SHOP then
        ...
        if self:handle_shop_touch(x, y) then
            return
        end
        self:clear_shop_selection()
        return
    end
    local pack_hand_move = (
        self.STATE == self.STATES.OPEN_BOOSTER
        and self.booster_session
        and self.booster_session.hand_for_tarot
    )
    ...
    if not selecting_hand and not joker_touch_state and not consumable_touch_state then
        return
    end
    ...
    if
        node
        and joker_touch_state
        and not pack_hand_move
        and (not node_is_owned_joker(self, node))
        and (not node_is_owned_consumable(self, node))
    then
        node = nil
    end
...
function Game:touchmoved(id, x, y, dx, dy)
    ...
    local zone_drag_state = (self.STATE == self.STATES.SHOP or self.STATE == self.STATES.OPEN_BOOSTER)
        and self.dragging
        and node_is_zone_draggable(self, self.dragging)
    local owned_joker_drag = self.jokers_on_bottom == true
        and self.dragging
        and node_is_owned_joker(self, self.dragging)
    local owned_cons_drag = self.consumables_on_bottom == true
        and self.dragging
        and select(1, node_is_owned_consumable(self, self.dragging))
    if
        not selecting_hand
        and not joker_touch_state
        and not consumable_touch_state
        and not zone_drag_state
        and not owned_joker_drag
        and not owned_cons_drag
    then
        return
    end
    if selecting_hand and self.hand and self.hand.is_scoring_active and self.hand:is_scoring_active() then
    ...
    if
        self.dragging
        and select(1, node_is_owned_consumable(self, self.dragging))
        and self.consumables_on_bottom == true
    then
        if self.dragging.touchmoved then
            self.dragging:touchmoved(id, x, y, dx, dy)
        end
    ...
    if self.dragging and joker_touch_state then
        if
            not node_is_owned_joker(self, self.dragging)
            and not (pack_hand_move and node_is_hand_card(self, self.dragging))
        then
            return
        end
    end
...
function Game:touchreleased(id, x, y)
    ...
    if
        not selecting_hand
        and not joker_touch_state
        and not tapped_consumable
        and not shop_offer_touch_state
        and not zone_drag_touch_state
    then
        self.dragging = nil
        return
    end
    ...
    if
        joker_touch_state
        and self.dragging
        and not node_is_owned_joker(self, self.dragging)
        and not node_is_zone_draggable(self, self.dragging)
    then
    ...
    if released then
        ...
        if ctx then
            ...
            local is_merchandise = ctx.kind == "shop_offer"
                or ctx.kind == "shop_booster"
                or ctx.kind == "shop_voucher"
                or ctx.kind == "booster_choice"
    ...
    if
        not zone_action_done
        and released
        and self.jokers
        and self.jokers_on_bottom
        and node_is_owned_joker(self, released)
    then
    ...
    if
        released
        and self.STATE ~= self.STATES.SHOP
        and self.jokers_on_bottom
        and node_is_owned_joker(self, released)
        and not reordered
        and not zone_action_done
        and dist < TAP_THRESHOLD
    then
        local was_selected = (self.active_tooltip_joker == released)
        self:clear_bottom_tooltips()
        if not was_selected then
    ...
    if
        not zone_action_done
        and dist < TAP_THRESHOLD
        and self.STATE == self.STATES.OPEN_BOOSTER
        and released
        and node_is_booster_choice(released)
    then
...
function Game:restore_hand_draw_order()
    if not self.hand or #self.hand.card_nodes == 0 then
        return
    end
    local hand_set = {}
    for _, node in ipairs(self.hand.card_nodes) do
...
function Game:sync_gamepad_focus_after_inventory_change()
    if
        self.STATE == self.STATES.SELECTING_HAND
        or self.STATE == self.STATES.SHOP
        or self.STATE == self.STATES.OPEN_BOOSTER
    then
...
function Game:is_shop_item_selected(node)
    if self.STATE ~= self.STATES.SHOP or not node then
        return false
    end
    if node.shop_offer_slot and self.active_tooltip_joker == node then
        return true
    end
    if
        node.shop_voucher_slot
        and tonumber(self.active_tooltip_shop_voucher_slot) == tonumber(node.shop_voucher_slot)
    then
        return true
    end
...
function Game:shop_joker_selection_lift_y(node)
    ...
    if self:is_gamepad_joker_focused(node) then
        return -8
    end
    if not self:is_shop_item_selected(node) then
        return 0
    end
...
function Game:joker_gamepad_focus_at(idx)
    if not self.jokers or #self.jokers == 0 then
        return nil
    end
...
function Game:consumable_reorder_gamepad_step(delta)
    ...
    local idx = tonumber(self._consumable_focus_index)
    if not idx or not self.consumable_nodes or not self.consumables then
        return false
    end
    delta = math.floor(tonumber(delta) or 0)
    if delta == 0 then
    ...
    if to_idx < 1 or to_idx > #self.consumable_nodes then
        return false
    end
    local node = table.remove(self.consumable_nodes, from_idx)
    table.insert(self.consumable_nodes, to_idx, node)
...
function Game:joker_reorder_gamepad_step(delta)
    if not self.jokers_on_bottom or not self.jokers then
        return false
    end
    local idx = tonumber(self._joker_focus_index)
    if not idx then
    ...
    local from_idx = idx
    local to_idx = from_idx + delta
    if to_idx < 1 or to_idx > #self.jokers then
        return false
    end
    local node = table.remove(self.jokers, from_idx)
    table.insert(self.jokers, to_idx, node)
...
function Game:is_joker_swap_pick(joker)
    ...
    if not pick or not self.jokers then
        return false
    end
...
function Game:draw_node_gamepad_focus_outline(node)
    if not node or not self:should_draw_gamepad_focus_outline(node) then
        return
    end
...
function Game:_bottom_inventory_nav_active()
    ...
    if
        layer == "consumables"
        and self.consumables_on_bottom
        and self.consumable_nodes
        and #self.consumable_nodes > 0
    then
        return true
    end
    return false
...
function Game:set_dpad_cursor_for_node(node)
    if not self.hand or not self.hand.card_nodes or not node then
        return
    end
...
function Game:_dpad_horizontal_dir()
    if not self:_gamepad_horizontal_nav_active() then
        return 0
    end
    ...
    if love.keyboard.isDown then
        if love.keyboard.isDown("left") or love.keyboard.isDown("l") then
            return -1
        end
        if love.keyboard.isDown("right") or love.keyboard.isDown("r") then
            return 1
        end
    end
    return 0
...
function Game:_sweep_toggle_card(node)
    if not self.hand or not node then
        return
    end
...
function Game:ensure_sweep_seed()
    if not self:is_sweep_select_mode() or self._y_sweep_seeded then
        return
    end
    local node = self:dpad_cursor_node()
    if node then
...
function Game:_dpad_horizontal_step(dir, sweep)
    ...
    if not node or not self.hand then
        return
    end
    if sweep then
...
function Game:is_hand_reorder_mode()
    if not self:is_role_held("confirm") then
        return false
    end
    if not self:is_hand_cursor_active() then
        return false
...
function Game:is_joker_reorder_mode()
    if not self:is_role_held("confirm") or self.jokers_on_bottom ~= true then
        return false
    end
...
function Game:is_consumable_reorder_mode()
    if not self:is_role_held("confirm") or self.consumables_on_bottom ~= true then
        return false
    end
...
function Game:is_sweep_select_mode()
    if not self:is_role_held("sort") then
        return false
    end
...
function Game:move_selected_hand_cards_to_front()
    if not self.hand or #self.hand.selected == 0 then
        return
    end
    local sel = {}
    for _, n in ipairs(self.hand.selected) do
        sel[n] = true
    end
    local ordered = {}
    for _, node in ipairs(self.nodes) do
...
function Game:ensure_joker_sprite_loaded(key)
    ...
    if type(self.JOKER_SPRITES) ~= "table" then
        self.JOKER_SPRITES = {}
    end
    ...
    if not entry then
        entry = {
            name = key,
            path = "Assets/textures/1x/Jokers/" .. key .. ".png",
            image = nil,
...
function Game:unload_joker_sprite(key)
    if not key or type(self.JOKER_SPRITES) ~= "table" then
        return false
    end
    local entry = self.JOKER_SPRITES[key]
...
function Game:ensure_asset_atlas_loaded(name)
    if not name or not self.ASSET_ATLAS then
        return nil
    end
...
function Game:ensure_animation_atlas_loaded(name)
    if not name or not self.ANIMATION_ATLAS then
        return nil
    end
...
function Game:set_render_settings()
    ...
    self.animation_atli = {
        { name = "blind_chips", path = "Assets/textures/1x/BlindChips.png", px = 36, py = 36, frames = 21 },
        { name = "shop_sign", path = "Assets/textures/1x/ShopSignAnimation.png", px = 113, py = 60, frames = 4 },
        { name = "menu", path = "Assets/textures/1x/menu.png", px = 128, py = 128, frames = 63 },
    }
    self.asset_atli = {
        { name = "cards_1", path = "Assets/textures/1x/8BitDeck.png", px = 72, py = 95 },
        { name = "cards_2", path = "Assets/textures/1x/8BitDeck_opt2.png", px = 72, py = 95 },
        { name = "centers", path = "Assets/textures/1x/Enhancers.png", px = 72, py = 95 },
        { name = "Tarot", path = "Assets/textures/1x/Tarots.png", px = 64, py = 96 },
        { name = "Voucher", path = "Assets/textures/1x/Vouchers.png", px = 72, py = 95 },
        { name = "Booster", path = "Assets/textures/1x/boosters.png", px = 72, py = 95 },
        { name = "ui_1", path = "Assets/textures/1x/ui_assets.png", px = 18, py = 18 },
        { name = "ui_2", path = "Assets/textures/1x/ui_assets_opt2.png", px = 18, py = 18 },
        { name = "balatro", path = "Assets/textures/1x/balatro.png", px = 336, py = 216 },
        { name = "gamepad_ui", path = "Assets/textures/1x/gamepad_ui.png", px = 32, py = 32 },
        { name = "icons", path = "Assets/textures/1x/icons.png", px = 66, py = 66 },
        { name = "tags", path = "Assets/textures/1x/tags.png", px = 34, py = 34 },
        { name = "stickers", path = "Assets/textures/1x/stickers.png", px = 72, py = 95 },
        { name = "chips", path = "Assets/textures/1x/chips.png", px = 30, py = 30 },
    }
    self.asset_images = {
        { name = "playstack_logo", path = "Assets/textures/1x/playstack-logo.png", px = 1416, py = 1416 },
        { name = "localthunk_logo", path = "Assets/textures/1x/localthunk-logo.png", px = 1390, py = 560 },
    }
    ...
    for i = 1, #self.animation_atli do
        ...
        self.ANIMATION_ATLAS[name].image = eager
                and load_image(entry.path, { dpiscale = self.SETTINGS.GRAPHICS.texture_scaling, mipmaps = false })
            or nil
    end
