...
function M.tooltip_color_by_key(color_key)
    ...
    if color_key == "MULT" then
        return C.MULT or { 0.9, 0.3, 0.4, 1 }
    end
    if color_key == "CHIPS" then
        return C.CHIPS or { 0.3, 0.7, 1, 1 }
    end
    if color_key == "CHANCE" then
        return C.CHANCE or C.GREEN or { 0.2, 0.75, 0.55, 1 }
    end
    if color_key == "PURPLE" then
        return C.PURPLE or { 0.66, 0.51, 0.82, 1 }
    end
    if color_key == "IMPORTANT" then
        return C.IMPORTANT or { 1, 0.6, 0.0, 1 }
    end
    if color_key == "MONEY" then
        return C.MONEY or { 0.9, 0.8, 0.2, 1 }
    end
    if color_key == "RED" then
        return C.RED or { 0.996, 0.373, 0.333, 1 }
    end
    return { 0.22, 0.24, 0.26, 1 }
end
...
function M.draw_tooltip_layout(font, title, resolved_lines, draw_x, draw_y, anchor_w, anchor_h)
    ...
    local first_is_rarity = #resolved_lines > 0 and resolved_lines[1][1] and resolved_lines[1][1].rarity_badge == true
