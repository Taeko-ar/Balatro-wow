...
function Moveable:touchpressed(id, x, y)
    if not self.states.drag.can then
        return
    end
...
function Moveable:update(dt)
    ...
    if vel_mag > 2 then
        ...
        if tilt > math.pi then
            tilt = tilt - 2 * math.pi
        end
        if tilt > max_tilt then
            tilt = max_tilt
        elseif tilt < -max_tilt then
            tilt = -max_tilt
        end
    else
        tilt = 0
...
function Moveable:draw_boundingrect()
    ...
    love.graphics.rectangle("line", draw_x, draw_y, self.VT.w * self.VT.scale, self.VT.h * self.VT.scale)
