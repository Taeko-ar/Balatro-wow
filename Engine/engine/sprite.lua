...
function Sprite:set_from_index(index)
    self.index = index
    if not self.atlas or not self.atlas.image then
        return
    end

    local iw, _ = self.atlas.image:getDimensions()
...
function Sprite:draw_sprite()
    if not self.sprite.image or not self.sprite.quad then
        return
    end
    ...
    love.graphics.draw(self.sprite.image, self.sprite.quad, draw_x, draw_y, 0, scale_x, scale_y)
