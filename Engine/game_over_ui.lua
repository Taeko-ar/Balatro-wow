...
function GameOverUI.draw_bottom(game)
    ...
    love.graphics.printf(
        "You ran out of hands before beating this blind.",
        panel_x + 8,
        panel_y + 78,
        panel_w - 16,
        "left"
    ...
    love.graphics.rectangle(
        "fill",
        game._game_over_continue_rect.x,
        game._game_over_continue_rect.y,
        game._game_over_continue_rect.w,
        game._game_over_continue_rect.h,
        3,
        3
    )
    love.graphics.setFont(game.FONTS.PIXEL.SMALL)
    love.graphics.setColor(game.C.WHITE)
    local cty = game._game_over_continue_rect.y
        + math.floor((game._game_over_continue_rect.h - love.graphics.getFont():getHeight()) * 0.5 + 0.5)
...
function GameOverUI.handle_touch(game, x, y)
    if not game._game_over_continue_rect then
        return false
    end
