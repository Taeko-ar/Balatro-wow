...
function InputBindings.get_role_slot_button(role, slot, bindings)
    ...
    if slot < 1 or slot > InputBindings.SLOTS_PER_ROLE then
        return nil
    end
    bindings = bindings or copy_bindings(InputBindings.DEFAULT_BINDINGS)
...
function InputBindings.get_role_for_button(button, bindings)
    if type(button) ~= "string" or button == "" then
        return nil
    end
    bindings = bindings or copy_bindings(InputBindings.DEFAULT_BINDINGS)
    for _, role in ipairs(InputBindings.ROLES) do
...
function InputBindings.is_menu_activate(button, bindings)
    return InputBindings.is_role(button, "confirm", bindings) or InputBindings.is_role(button, "sort", bindings)
end

function InputBindings.is_menu_back(button, bindings)
    return InputBindings.is_role(button, "cancel", bindings) or InputBindings.is_role(button, "use", bindings)
end
...
function InputBindings.is_trigger_button(button)
    return type(button) == "string" and InputBindings.TRIGGER_BUTTONS[button] == true
end

function InputBindings.safe_is_trigger_down(joy, button)
    if not InputBindings.is_trigger_button(button) then
        return false
    end
    if not joy or not joy.getGamepadAxis then
        return false
    end
    local axes = { button }
...
function InputBindings.safe_is_gamepad_down(joy, button)
    ...
    if not joy or not joy.isGamepad or not joy:isGamepad() then
        return false
    end
    if type(button) ~= "string" or button == "" then
        return false
