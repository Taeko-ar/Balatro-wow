...
function Card:sync_visual_from_card_data()
    ...
    if enh == "none" or enh == "" then
        enh = nil
    end
    self.enhancement = enh

    local seal = data.seal
    if seal == "none" or seal == "" then
        seal = nil
    end
    self.seal = seal
...
function Card:draw_layer(atlas, quad, cell_w, cell_h, ox, oy)
    if not atlas or not atlas.image or not quad then
        return
    end
    ...
    love.graphics.draw(atlas.image, quad, draw_x, draw_y, 0, scale_x, scale_y)
end
...
local function enhancement_tooltip_lines(enh)
    if not enh or enh == "none" then
        return {}
    end
    if enh == "bonus" then
        return { "+30 chips" }
    elseif enh == "mult" then
        return { "+4 mult" }
    elseif enh == "glass" then
        return { "×2 mult", "1/4: breaks after score" }
    elseif enh == "steel" then
        return { "×1.5 mult while held" }
    elseif enh == "stone" then
        return { "+50 chips" }
    elseif enh == "gold" then
        return { "+$3 while held" }
    elseif enh == "lucky" then
        return { "1/5: +20 mult", "1/15: +$20" }
    elseif enh == "wild" then
        return { "Can be any Suit" }
    end
    return {}
end
...
local function seal_tooltip_lines(seal)
    ...
    if seal == "gold" then
        return { "+$3 when scored" }
    elseif seal == "red" then
        return { "Retriggers Card Once" }
    elseif seal == "blue" then
        return { "Creates a Planet card for the winning Hand if held in hand" }
    elseif seal == "purple" then
        return { "Creates a Tarot Card when Discarded" }
    end
    return {}
end
...
function Card:draw_tooltip(draw_x, draw_y)
    ...
    if header_only then
        ...
        local suit_col = (G and G.C and G.C.SUITS and G.C.SUITS[suit_name])
            or (G and G.C and G.C.PANEL)
            or { 1, 1, 1, 1 }
...
function Card:should_draw_tooltip()
    ...
    if G and G.is_hand_scoring_active and G:is_hand_scoring_active() then
        return false
    end
    if G and G._collection_open and G._collection_tooltip_node == self then
        return true
    end
...
function Card:draw()
    ...
    if G and G.is_hand_cursor_active and G:is_hand_cursor_active() and G.hand and G._dpad_cursor_index then
        ...
        if cursor == self then
            local rect = self:get_collision_rect()
    ...
    if self.enhancement and self.enhancement ~= "none" and self.do_enhancement then
        ...
        elseif
            (
                ...
                or self.enhancement == "mult"
                or self.enhancement == "glass"
                or self.enhancement == "lucky"
                or self.enhancement == "stone"
            ) and ev == "card_played"
        then
            self:do_enhancement(ctx)
            trigger = true
