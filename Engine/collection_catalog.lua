...
function CollectionCatalog.discovery_id_for_entry(entry)
    ...
    if entry.discovery_id then
        return entry.discovery_id
    end
    return entry.id
end

function CollectionCatalog.is_entry_discovered(game, entry)
    if not game or not entry then
        return false
    end
...
function CollectionCatalog.entry_tooltip_content(game, entry)
    ...
    elseif kind == "enhanced" then
        return entry.name or "Enhanced Card", CollectionCatalog.ENHANCEMENT_TOOLTIPS[entry.enhancement] or {}
    elseif kind == "seal" then
        return entry.name or "Seal", CollectionCatalog.SEAL_TOOLTIPS[entry.seal] or {}
