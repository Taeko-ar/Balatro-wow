...
function RoundWinUI.draw_bottom(game, payout_lines)
    ...
    love.graphics.rectangle(
        "fill",
        game._round_win_continue_rect.x,
        game._round_win_continue_rect.y,
        game._round_win_continue_rect.w,
        game._round_win_continue_rect.h,
        3,
        3
    )
    love.graphics.setFont(game.FONTS.PIXEL.SMALL)
    love.graphics.setColor(game.C.WHITE)
    local cty = game._round_win_continue_rect.y
        + math.floor((game._round_win_continue_rect.h - love.graphics.getFont():getHeight()) * 0.5 + 0.5)
...
function RoundWinUI.handle_touch(game, x, y)
    if not game._round_win_continue_rect then
        return false
    end
