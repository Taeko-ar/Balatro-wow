...
function ShopUI.layout_shop_booster_nodes(game, param)
    ...
    if n <= 0 or type(param) ~= "table" then
        return
    end
...
function ShopUI.draw_shop_offer_price_tags(game)
    if game.STATE ~= game.STATES.SHOP then
        return
    end
...
function ShopUI.draw_booster_atlas_frame(game, rect, frame_index_zero_based)
    if not game or not rect or type(frame_index_zero_based) ~= "number" then
        return false
    end
    ...
    if qx + px > iw + 0.5 or qy + py > ih + 0.5 then
        return false
    end
...
function ShopUI.draw_bottom_shop(game)
    ...
    game._shop_continue_rect =
        { x = shop_continue_rect.x, y = shop_continue_rect.y, w = shop_continue_rect.w, h = shop_continue_rect.h }
    game._shop_reroll_rect =
        { x = shop_reroll_rect.x, y = shop_reroll_rect.y, w = shop_reroll_rect.w, h = shop_reroll_rect.h }
    ShopUI.draw_shop_button(game, shop_continue_rect)
    ShopUI.draw_shop_button(game, shop_reroll_rect)

    love.graphics.setColor(game.C.PANEL)
    local jokerPanel = {
        x = shop_continue_rect.x + shop_continue_rect.w + padding,
        y = shop_continue_rect.y,
        w = panel_w - 3 * padding - shop_continue_rect.w,
        h = (shop_reroll_rect.y + shop_reroll_rect.h) - shop_continue_rect.y,
    }
    ...
    love.graphics.print(
        text,
        panel_x - 1,
        voucherPanel.y + love.graphics.getFont():getWidth(text) / 2 + voucherPanel.h / 2,
        math.rad(-90)
    )
end
