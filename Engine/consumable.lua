...
function Consumable:init(X, Y, def)
    ...
    if (not self.atlas or not self.atlas.image or not self.quad) and G and self.atlas_name then
        ...
        if not _consumable_missing_atlas_reported[key] then
            ...
            local err = (self.atlas and self.atlas.load_error) and tostring(self.atlas.load_error)
                or "unknown atlas/quad failure"
            print(
                string.format(
                    "[Consumable] draw fallback for '%s' atlas='%s' idx=%s err=%s",
                    tostring(self.name),
                    tostring(self.atlas_name),
                    tostring(self.index),
                    err
                )
            )
...
function Consumable:draw_tooltip_overlay()
    if not self.states.visible or not self:tooltip_is_active() then
        return
    end
    local draw_x = self.VT.x + self.collision_offset.x
    local draw_y = self.VT.y + self.collision_offset.y
