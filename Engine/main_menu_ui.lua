...
function MainMenuUI.draw_main(game)
    ...
    if _G.draw_rect_with_shadow then
        draw_rect_with_shadow(panel_x, panel_y, panel_w, panel_h, 6, 3, game.C.PANEL, game.C.BLOCK.SHADOW, 3)
    else
    ...
    if has_save then
        ...
        draw_rect_with_shadow(
            game._main_menu_continue_rect.x,
            game._main_menu_continue_rect.y,
            game._main_menu_continue_rect.w,
            game._main_menu_continue_rect.h,
            4,
            4,
            game.C.BLUE,
            game.C.BLOCK.SHADOW,
        ...
        draw_rect_with_shadow(
            game._main_menu_start_rect.x,
            game._main_menu_start_rect.y,
            game._main_menu_start_rect.w,
            game._main_menu_start_rect.h,
            4,
            4,
            game.C.GREEN,
            game.C.BLOCK.SHADOW,
        ...
        draw_rect_with_shadow(
            game._main_menu_how_to_play_rect.x,
            game._main_menu_how_to_play_rect.y,
            game._main_menu_how_to_play_rect.w,
            game._main_menu_how_to_play_rect.h,
            4,
            4,
            game.C.MULT,
            game.C.BLOCK.SHADOW,
        ...
        draw_rect_with_shadow(
            game._main_menu_collection_rect.x,
            game._main_menu_collection_rect.y,
            game._main_menu_collection_rect.w,
            game._main_menu_collection_rect.h,
            4,
            4,
            game.C.ORANGE or game.C.BLUE,
            game.C.BLOCK.SHADOW,
            2
        )
        ...
        love.graphics.printf(
            "Continue Run",
            game._main_menu_continue_rect.x,
            btn_label_y(game._main_menu_continue_rect),
            game._main_menu_continue_rect.w,
            "center"
        )
        ...
        love.graphics.printf(
            "New Run",
            game._main_menu_start_rect.x,
            btn_label_y(game._main_menu_start_rect),
            game._main_menu_start_rect.w,
            "center"
        )
        ...
        love.graphics.printf(
            "How to Play",
            game._main_menu_how_to_play_rect.x,
            btn_label_y(game._main_menu_how_to_play_rect),
            game._main_menu_how_to_play_rect.w,
            "center"
        )
        ...
        love.graphics.printf(
            "Collection",
            game._main_menu_collection_rect.x,
            btn_label_y(game._main_menu_collection_rect),
            game._main_menu_collection_rect.w,
            "center"
        )
    else
    ...
    for i = 1, profile_count do
        ...
        game._main_menu_profile_rects[i] =
            { x = profile_btn_x, y = profile_btn_y, w = profile_btn_w, h = profile_btn_h }
    ...
    love.graphics.printf(
        delete_label,
        profile_btn_x,
        profile_btn_y + math.floor((profile_btn_h - love.graphics.getFont():getHeight()) * 0.5 + 0.5),
        profile_btn_w,
        "center"
    )
...
function MainMenuUI._start_run(game)
    ...
    if not deck_def or not game:is_deck_unlocked(deck_def.id) then
        return false
    end
    if not stake_def or not game:is_stake_unlocked(deck_def.id, stake_def.id) then
        return false
    end
...
function MainMenuUI.draw_deck_select(game)
    ...
    local next_x = W - 52
    local card_x, card_y, card_w, card_h =
        startX + buttonW + padding, startY + padding, frameW - 2 * buttonW - 3 * padding, frameH - 4 * padding
    ...
    local dx, _, dw, _ = MainMenuUI.draw_deck_carousel_sprite(game, def, card_x + 4, card_y, 64, card_h, 0)
    ...
    love.graphics.rectangle(
        "fill",
        infoX + padding,
        infoY + padding + offset,
        infoW - 2 * padding,
        infoH - 2 * padding - offset,
        4,
        4
    )
    ...
    love.graphics.setColor(C.PANEL)
    love.graphics.printf(
        def.unlocked and (def.description or "")
            or def.unlock_condition and def.unlock_condition.text
            or "Complete the unlock condition to play this deck.",
        infoX + 2 * padding,
        infoY + 2 * padding + offset,
        infoW - 4 * padding,
        "center"
    )
    ...
    for i = 1, #stake_list do
        ...
        love.graphics.rectangle(
            "fill",
            markerX,
            markerY2 - math.floor(markerH / 2) - space - (markerH + space) * (i - 1),
            20,
            markerH,
            4,
            4
    ...
    for i = 1, #deck_list do
        ...
        local x = midX + dot_offset * (i - mid - 1)
        if sel_idx == i then
            love.graphics.setColor(C.WHITE)
        else
            love.graphics.setColor(C.BLOCK.BACK)
    ...
    local sx, _, sw, _ =
        MainMenuUI.draw_stake_carousel_sprite(game, stake_def, card_x - 12, card_y, 64, stakeH - 2 * padding, 0)
    ...
    love.graphics.rectangle(
        "fill",
        stake_info_x + padding,
        stake_info_y + padding + stake_name_h,
        stake_info_w - 2 * padding,
        stake_info_h - 2 * padding - stake_name_h,
        4,
        4
    )
    ...
    love.graphics.printf(
        ...
        stake_info_x + 2 * padding,
        stake_info_y + padding + stake_name_h,
        stake_info_w - 4 * padding,
        "center"
    )
    ...
    for i = 1, #stake_list do
        local x = stake_mid_x + dot_offset * (i - math.floor(#stake_list / 2) - 1)
        if stake_idx == i then
            love.graphics.setColor(C.WHITE)
        else
            love.graphics.setColor(C.BLOCK.BACK)
