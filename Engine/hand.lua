...
function Hand:play_sfx_trigger()
    Sfx.play("Assets/sounds/generic1.ogg")
    Sfx.play_chips()
end
...
function Hand:add_card(card_data, bypass_limit)
    if not card_data or not self.game then
        return nil
    end
    ...
    if not bypass_limit and #self.cards >= limit then
        return nil
    end
    ...
    Sfx.play_random("Assets/sounds/cardSlide1.ogg", "Assets/sounds/cardSlide2.ogg")
...
function Hand:try_reorder_card_after_drag(node, release_x)
    if self._play_sequence or not node then
        return false
    end
    local from_idx
...
function Hand:reorder_node_step(node, delta)
    ...
    if to_idx < 1 or to_idx > #self.card_nodes then
        return false
    end
...
function Hand:layout(update_visual, skip_vt_node)
    ...
    if update_visual == nil then
        update_visual = true
    end

    if
        self.game
        and self.game.STATE == self.game.STATES.OPEN_BOOSTER
        and self.game.booster_session
        and self.game.booster_session.hand_for_tarot
    then
...
function Hand:toggle_selection(node)
    ...
    if not node or not self.game then
        return
    end
    for i, n in ipairs(self.selected) do
        if n == node then
            if
                self.game
                and self.game.get_active_boss_blind_id
                and self.game:get_active_boss_blind_id() == "bl_final_bell"
            then
            ...
            node.selected = false
            Sfx.play("Assets/sounds/card3.ogg")
            ...
            if self.game.move_selected_hand_cards_to_front then
                self.game:move_selected_hand_cards_to_front()
            end
            self:calculate_play()
            return
    ...
    if #self.selected >= MAX_SELECTED then
        return
    end
    node.selected = true
    Sfx.play("Assets/sounds/card1.ogg")
...
function Hand:discard_selected()
    ...
    if #self.selected == 0 or not self.game or G.discards <= 0 then
        return
    end
...
function Hand:build_contained_hand_types(nodes)
    ...
    if max_of_a_kind >= 2 then
        contained["Pair"] = true
    end
    if pairs_count >= 2 or (max_of_a_kind >= 3 and pairs_count >= 1) then
        contained["Two Pair"] = true
    end
    if max_of_a_kind >= 3 then
        contained["Three of a Kind"] = true
    end
    if max_of_a_kind >= 4 then
        contained["Four of a Kind"] = true
    end
    if flush then
        contained["Flush"] = true
    end
    if straight then
        contained["Straight"] = true
    end
                ...
                if node and score_this then
                    ...
                    if G and G.begin_joker_emit and G:begin_joker_emit("card_played", jctx) then
                        ...
                        seq.joker_wait_resume =
                            { phase = "trigger", advance_play_repeat = true, delay_next_trigger = true }
                    else
    ...
    elseif seq.phase == "finalize" then
        ...
        if seq.finalize_step == 2 then
            ...
            if (G._deck_special or nil) == "plasma" then
                ...
                chips = avg
                mult = avg
                Sfx.play("Assets/sounds/gong.ogg")
            end
...
function Hand:accumulate_card_score(chips, mult, node)
    ...
    local poly = type(data.modifier) == "table" and data.modifier.edition == "polychrome"
    if poly then
        mult = mult * 1.5
    end
...
function Hand:play_selected()
    if BalatroDebug then
        DEFAULT_CHAT_FRAME:AddMessage(
            "Balatro: Hand:play_selected() called. Selected count: "
                .. tostring(#self.selected)
                .. ", Hands left: "
                .. tostring(G.hands)
        )
    end
    if #self.selected == 0 or G.hands <= 0 then
        if BalatroDebug then
            DEFAULT_CHAT_FRAME:AddMessage("Balatro: Aborting play_selected. Selected count is 0 or no hands left.")
        end
        return
    end
    if self._play_sequence then
        if BalatroDebug then
            DEFAULT_CHAT_FRAME:AddMessage("Balatro: Aborting play_selected. Play sequence already active.")
        end
        return
    end
...
function Hand:calculate_play()
    ...
    local function is_flush()
        if n < min_straight_flush_cards then
            return false
        end
        if wild_count == n then
            return true
        end
        if next(suit_counts) == nil then
            return false
        end
        if next(suit_counts, next(suit_counts)) == nil then
            return true
        end
    ...
    local function is_straight()
        ...
        local function has_valid_run(sorted_unique_ranks)
            if #sorted_unique_ranks < min_straight_flush_cards then
                return false
            end
            local run_len = 1
        ...
        if has_valid_run(uniq_ranks) then
            return true
        end
