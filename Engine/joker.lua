...
function Joker.normalize_edition(raw)
    ...
    if e == "holographic" or e == "e_holo" then
        return "holo"
    end
    if e == "e_foil" or e == "foil" then
        return "foil"
    end
    if e == "e_polychrome" or e == "polychrome" then
        return "polychrome"
    end
    if e == "e_negative" or e == "negative" then
        return "negative"
    end
    if e == "holo" or e == "polychrome" or e == "negative" then
        return e
    end
    return "base"
end
...
local function compute_quad(atlas, index)
    if not atlas or not atlas.image or index == nil then
        return nil, 0, 0
    end
...
local function resolve_atlas(name)
    if not name or not G or not G.ASSET_ATLAS then
        return nil
    end
...
local function resolve_joker_sprite(key)
    if not key or not G or not G.ensure_joker_sprite_loaded then
        return nil
    end
...
function Joker:init(X, Y, W, H, def, params)
    ...
    if type(self.def) == "table" then
        ...
        elseif self.def.id == "j_ramen" then
            self.runtime_counter = self.def.config.Xmult or 2
        elseif self.def.id == "j_seltzer" then
            self.runtime_counter = self.def.config.duration or 10
        elseif self.def.id == "j_ice_cream" then
            self.runtime_counter = self.def.config.chips or 100
    ...
    if self.face_up == nil then
        self.face_up = true
    end

    self.front_atlas_name = (self.params.pos and self.params.pos.atlas)
        or (self.def.pos and self.def.pos.atlas)
        or "Joker1_p1"
...
local function joker_effect_kind(joker)
    ...
    if type(et) == "table" then
        if et.type == "add_mult" then
            return "Mult"
        end
        if et.type == "add_chips" then
            return "Chips"
        end
    end
    local cfg = joker.effect_config or {}
    if cfg.mult ~= nil then
        return "Mult"
    end
    if type(cfg.extra) == "table" and cfg.extra.s_mult ~= nil then
        return "Suit Mult"
    end
    if cfg.t_mult ~= nil then
        return "Type Mult"
    end
    if type(cfg.extra) == "table" and cfg.extra.s_chips ~= nil then
        return "Suit Chips"
    end
    if cfg.t_chips ~= nil then
        return "Type Chips"
    end
    return nil
end
...
function Joker:get_live_current_tooltip_text(base_text)
    ...
    local multipliers = {
        ...
        j_steel_joker = function(_)
            local steel = count_full_deck(function(c)
                return c.enhancement == "steel"
            end)
            local x = 1 + (0.2 * steel)
            return "(Currently X" .. fmt_runtime_number(x, 2) .. " Mult)"
        end,
        j_constellation = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        j_madness = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        j_vampire = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        j_hologram = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        j_obelisk = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        ...
        j_lucky_cat = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        j_campfire = function(j)
            return "(Currently X" .. fmt_runtime_number(j.stored_xmult or 1, 2) .. " Mult)"
        end,
        j_ramen = function(j)
    ...
    local mults = {
        j_ceremonial = function(j)
            return string.format("(Currently +%d Mult)", math.floor(tonumber(j.stored_mult) or 0))
        end,
        j_abstract = function()
            return string.format("(Currently +%d Mult)", 3 * ((G and G.jokers and #G.jokers) or 0))
        end,
        j_ride_the_bus = function(j)
            return string.format("(Currently +%d Mult)", math.floor(tonumber(j.runtime_counter) or 0))
        end,
        j_green_joker = function(j)
            return string.format("(Currently +%d Mult)", math.floor(tonumber(j.stored_mult) or 0))
        end,
        j_red_card = function(j)
            return string.format("(Currently +%d Mult)", math.floor(tonumber(j.stored_mult) or 0))
        end,
        j_erosion = function()
        ...
        j_bootstraps = function()
            return string.format("(Currently +%d Mult)", math.floor((tonumber(G and G.money) or 0) / 5) * 2)
        end,
        j_flash_card = function(j)
            return string.format("(Currently +%d Mult)", math.floor(tonumber(j.stored_mult) or 0))
        end,
        j_spare_trousers = function(j)
            return string.format("(Currently +%d Mult)", math.floor(tonumber(j.stored_mult) or 0))
        end,
        j_fortune_teller = function()
            return string.format("(Currently +%d)", math.floor(tonumber(G and G.tarots_used) or 0))
        end,
        j_popcorn = function(j)
            return string.format("+%d Mult", math.floor(tonumber(j.stored_mult) or 0))
        end,
    }
    if mults[id] then
    ...
    local chips = {
        ...
        j_runner = function(j)
            return string.format("(Currently +%d Chips)", math.floor(tonumber(j.stored_chips) or 0))
        end,
        j_blue_joker = function()
            return string.format("(Currently +%d Chips)", 2 * count_cards_in_deck())
        end,
        j_square = function(j)
            return string.format("(Currently +%d Chips)", math.floor(tonumber(j.stored_chips) or 0))
        end,
        ...
        j_stone_joker = function()
            return string.format("(Currently +%d Chips)", 25 * count_full_deck(function(c)
                return c.enhancement == "stone"
            end))
        end,
        j_bull = function()
            return string.format("(Currently +%d Chips)", 2 * (tonumber(G and G.money) or 0))
        end,
    }
    if chips[id] then
...
function Joker:get_edition_tooltip_lines()
    ...
    if ed == "foil" then
        return { { kind = "text", text = "Foil: +50 Chips when hand is scored" } }
    end
    if ed == "holo" then
        return { { kind = "text", text = "Holographic: +10 Mult when hand is scored" } }
    end
    if ed == "polychrome" then
        return { { kind = "text", text = "Polychrome: ×1.5 Mult when hand is scored" } }
    end
    if ed == "negative" then
        return { { kind = "text", text = "Negative: +1 Joker slot" } }
    end
    return {}
end
...
function Joker:get_layout_draw_xy()
    ...
    if G and self.shop_offer_slot == nil and G.jokers_on_bottom then
        local lifted = (G.active_tooltip_joker == self) or (G.is_joker_swap_pick and G:is_joker_swap_pick(self))
        if lifted then
            draw_y = draw_y - 8
        end
    end
...
function Joker:should_draw_tooltip()
    if not self.face_up or not G then
        return false
    end
    if G.is_hand_scoring_active and G:is_hand_scoring_active() then
        return false
    end
    if G._collection_open and G._collection_tooltip_node == self then
        return true
    end
    if G.is_card_select_mode and G:is_card_select_mode() then
        return false
    end
    if G.STATE == G.STATES.BLIND_SELECT and G.active_tooltip_skip_blind_index then
        return false
    end
    ...
    return G.active_tooltip_joker == self and (G.jokers_on_bottom == true or self.shop_offer_slot ~= nil)
end

function Joker:draw_tooltip_overlay()
    if not self.states.visible or not self:should_draw_tooltip() then
        return
    end
...
function Joker:draw_sub_pos_overlay(draw_x, draw_y)
    ...
    if not self.sub_atlas or not self.sub_atlas.image or not self.sub_quad then
        return
    end
...
function Joker:draw_sticker_overlays(draw_x, draw_y)
    if not self.sticker_atlas or not self.sticker_atlas.image then
        return
    end

    local active_stickers = {}
    if self.perishable then
        table.insert(active_stickers, "perishable")
    end
    if self.rental then
        table.insert(active_stickers, "rental")
    end
    if self.eternal then
        table.insert(active_stickers, "eternal")
    end
    if #active_stickers == 0 then
        return
    end
...
function Joker:update(dt)
    ...
    if self.scoring_shake_timer and self.scoring_shake_timer > 0 then
        ...
        if self.scoring_shake_timer < 0 then
            self.scoring_shake_timer = 0
        end
        if self.scoring_shake_timer <= 0 then
            self.scoring_shake_phase = nil
        end
    end
end
