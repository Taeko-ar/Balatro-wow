...
local function joker_sprite_key_from_def(def, edition)
    if not def or not Joker or not Joker.sprite_key_from_pos then
        return nil
    end
    local pos = def.pos
...
local function draw_undiscovered_collection_card(x, y, w, h)
    if not G or not G.ensure_asset_atlas_loaded then
        return
    end
...
function CollectionStaticNode:draw_tooltip_overlay()
    if not G or G._collection_tooltip_node ~= self then
        return
    end
...
local function category_color(game, color_key)
    ...
    if color_key == "PURPLE" then
        return sec.Tarot or C.TAROT or { 0.65, 0.51, 0.82, 1 }
    end
    if color_key == "PLANET" then
        return sec.Planet or C.PLANET or { 0.07, 0.69, 0.81, 1 }
    end
    if color_key == "SPECTRAL" then
        return sec.Spectral or C.SPECTRAL or { 0.27, 0.52, 0.98, 1 }
    end
...
local function wrap_draw_undiscovered(node)
    if node._collection_draw_wrapped then
        return
    end
...
function CollectionUI.draw_category_menu(game)
    ...
    draw_collection_button(game._collection_back_rect, "Back", nil, C.MULT, C, font_s, font_s)
...
function CollectionUI.draw_grid(game)
    ...
    love.graphics.printf("LEFT/RIGHT: Pages   B/X: Back", 0, nav_y, W, "center")
end
...
function CollectionUI.handle_touchpressed(game, id, x, y)
    ...
    if CollectionUI.handle_touch_grid(game, x, y) then
        return true
    end
